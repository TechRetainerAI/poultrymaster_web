...
**If login suddenly 404s while pages still load,** don't touch env or ports — it is the
Turbopack dev route table, not your setup. Run `npm run dev:check`, then
`npm run dev:reset`. See Issue 5.
...
## Issue 5 — Login dies with "Non-JSON response, HTTP 404" after a big change

### Symptom

Logging in locally fails and the browser console shows:

```
[Login diagnostic] Non-JSON response, HTTP 404
Login flow: browser → http://localhost:3000/api/proxy/Authentication/login → server forwards to:
http://localhost:5010
```

The tell-tale detail: **ordinary pages still load fine** (`/login`, `/dashboard` all
return 200). Only `/api/proxy/*` 404s. It has a habit of appearing straight after a large
change — a batch of edited pages, a big merge — which makes it look like something you
just wrote broke the app.

Nothing you wrote broke the app, and the backend is not involved.

### Why

`.next/dev/types/routes.d.ts` is generated by Turbopack and lists every route Next knows
about. It gets written over itself **without being truncated first**, so it ends up with
two generations interleaved. Observed on 2026-09-16:

| | healthy | damaged |
|---|---|---|
| lines | 437 | **642** |
| `export type { AppRoutes ... }` footers | 1 | **2** |
| `"/api/` references | 8 | **0** |

642 ≈ 437 + 205: a longer older generation bleeding out past the end of a shorter newer
one. With zero `/api/*` entries, `app/api/proxy/[...path]/route.ts` is **not in the route
table at all**, so Next matches those requests against the *page* tree instead, falls
through to `/_not-found`, and returns its HTML 404 page. HTML is not JSON — hence
"Non-JSON response".

The 2026-09-15 occurrence also produced `TS1160: Unterminated template literal` errors
inside `.next/dev/types/`, which were dismissed as stale output at the time. They were
damage, not staleness.

**What is NOT the cause** (both tested on 2026-09-16 and ruled out):

- **Running `npm run build` while the dev server is up.** Suspected, then disproved —
  the build writes `.next/types/` and the dev server writes `.next/dev/types/`, and a
  full build with the dev server live left the dev table intact (8 refs, 1 footer, and
  the proxy still answering).
- **Two dev servers at once.** Also disproved for a small change: Next keeps a
  `.next/dev/lock`, which appears to serialise them.

So the precise trigger is still unidentified. It is a Next 16.1.0 / Turbopack
non-atomic-write fault, not something in this repo's configuration, and the honest
summary is: it happens, it is cheap to detect, and it is cheap to fix.

### Fix applied

Three things, so this costs a minute instead of an hour:

1. **`npm run dev:check`** — reports the route table and changes nothing. Exits 0 when
   healthy, 1 when damaged. Run this *first*, before you suspect your own code.

   ```
   before: 437 lines, 8 /api refs, 1 footers -> healthy
   ```

2. **`npm run dev:reset`** (`scripts/dev-reset.mjs`) — stops stray `next dev` processes
   for this project, deletes `.next/dev`, and starts a clean dev server.
   `npm run dev:reset -- --no-start` cleans up without starting.

   It deletes only `.next/dev`, **not** the whole of `.next`: the full tree is ~11k files
   and slow to delete on Windows, and `.next/` also holds the production build output that
   `server.js` (`distDir: '.next'`) and the Dockerfile (`.next/standalone`) depend on.

   It kills processes *before* deleting, because a live dev server holds file locks under
   `.next/dev` and the delete otherwise silently leaves files behind.

3. **The login diagnostic now names the real cause** (`lib/api/auth.ts`). It detects an
   HTML 404 from our own origin — which can only mean Next never routed the request to the
   proxy — and prints the route-table checklist instead of the upstream one. The old
   message pointed at Cloud Run logs and `NEXT_PUBLIC_ADMIN_API_URL`, which is the wrong
   direction entirely when Next 404s before the proxy ever runs. That misdirection cost
   real time twice, on 09-15 and again on 09-16.

### One more thing worth knowing: orphaned dev servers

Observed while fixing this on 2026-09-16. On Windows, stopping `next dev` (Ctrl+C, or
killing the `npm` and `next` parents) can leave the `next-server` child alive **still
holding port 3000**. Start a new dev server and it quietly binds **3001** instead, prints
`Suggestion: If you intended to restart next dev, terminate the other process`, and your
browser at `localhost:3000` keeps talking to the orphan — with whatever broken cache it
had. `npm run dev:reset` handles this: it frees port 3000 if the holder belongs to this
project.

If a change you just made appears to have no effect at all, check the port in the dev
server's banner before anything else.

### Prevention for people who pull

Nothing to configure. Just remember the order when `/api/proxy/*` 404s but pages load:

```
npm run dev:check     # is the route table damaged?
npm run dev:reset     # fix it
```

Do not start editing `.env.local`, ports or Cloud Run settings for this symptom — if
pages render, Next and your env are fine.
