...
export default function PoultryInternalUsePage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    getPoultryInternalUseSuggestedCost(form.poultryProductId)
      .then((r) => {
        if (cancelled) return
        setSuggestedPerBase(r.unitCost || 0)
        setForm((p) => ({
          ...p,
          unitCost: round4((r.unitCost || 0) * (p.entryUnit === "Crate" ? eggsPerCrate : 1)),
        }))
      })
      .catch(() => { /* suggestion only — leave whatever is in the field */ })
    return () => { cancelled = true }
  }, [form.poultryProductId, eggsPerCrate])

  /**
   * Switching crate ↔ egg rescales the cost so the field keeps meaning what its
   * label says. Without this, changing the unit would silently turn "15.00 per
   * crate" into "15.00 per egg" — a 30× error in the expense.
   */
  function changeEntryUnit(next: string) {
    setForm((p) => {
      if (p.entryUnit === next) return p
      const toCrate = next === "Crate"
      const rescaled = toCrate
        ? (p.unitCost || 0) * eggsPerCrate
        : (p.unitCost || 0) / Math.max(eggsPerCrate, 1)
