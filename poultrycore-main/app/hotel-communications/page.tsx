...
import { listCommunications, createCommunication, updateCommunicationStatus, listHotelGuests, listHotelBookings, type HotelGuest, type HotelBooking } from "@/lib/api/hotel"
...
export default function HotelCommunicationsPage() {
  ...
  async function load() { setLoading(true); try { const [c, g, b] = await Promise.all([listCommunications(), listHotelGuests(), listHotelBookings()]); setItems(c); setGuests(g); setBookings(b) } catch (e: any) { toast({ title: "Failed", description: e?.message, variant: "destructive" }) } finally { setLoading(false) } }
