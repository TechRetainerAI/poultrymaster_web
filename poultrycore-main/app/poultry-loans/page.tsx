...
// Poultry Loans.
//
// Who lent the farm money, how much is still owed, and what each repayment was
// actually made of.
//
...
// cost. The first is the farm swapping cash for a smaller debt, and counting it
...
export default function PoultryLoansPage() {
  ...
  const [payments, setPayments] = useState<PoultryLoanPayment[]>([])
  const [summary, setSummary] = useState<PoultryLoanSummary | null>(null)
  ...
  const [form, setForm] = useState({
    ...
    originalPrincipal: "0", amountReceived: "0", poultryCashAccountId: "",
  ...
  const load = async () => {
    ...
    try {
      const [accs, ls, ps, sum] = await Promise.all([
        listPoultryCashAccounts(), listPoultryLoans(), listPoultryLoanPayments(), getPoultryLoanSummary(),
      ])
  ...
  const saveLoan = async () => {
    ...
    if (recv > 0 && !form.poultryCashAccountId) {
    ...
    try {
      await createPoultryLoan({
        ...
        amountReceived: recv,
        poultryCashAccountId: recv > 0 ? Number(form.poultryCashAccountId) : null,
      ...
      toast({
        title: "Loan recorded",
        description: recv > 0
          ? `${fmt(recv)} in. It is money in, not revenue — the farm borrowed it.`
  ...
  const doReverse = async () => {
    ...
    setSaving(true)
    try {
      await reversePoultryLoanPayment(reversing.poultryLoanPaymentId, reason.trim())
      ...
      <Dialog open={newOpen} onOpenChange={setNewOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              The money arriving is cash in, but it is not revenue — the farm borrowed it and still
              owes it.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <FormSection title="The money" color="green">
              <FormField label="Amount borrowed *"
                         info="What the farm OWES. This is what the debt starts at.">
