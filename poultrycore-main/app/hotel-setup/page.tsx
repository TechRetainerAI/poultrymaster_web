...
import { Loader2, Plus, Settings, Trash2, Edit2, CalendarDays, DoorOpen, Search, BedDouble, Layers, Sparkles } from "lucide-react"
...
import {
  ...
  type HotelRoomCategory, type HotelBedType,
...
export default function HotelSetupPage() {
  ...
  async function loadAll() {
    setLoading(true)
    try {
      const [rc, bt, rt, fl, am, rr, rm] = await Promise.all([
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Work left to right — each step feeds the next</p>
              </div>
            </div>

            <Tabs defaultValue="floors" className="space-y-6">
              <TabsList className="bg-white border shadow-sm flex-wrap h-auto">
                <TabsTrigger value="floors" className="data-[state=active]:bg-violet-50 data-[state=active]:text-violet-700">
                  <Layers className="h-4 w-4 mr-2" /> Floors
                  {floors.length > 0 && <Badge variant="secondary" className="ml-2 h-5 px-1.5">{floors.length}</Badge>}
                </TabsTrigger>
                <TabsTrigger value="room-types" className="data-[state=active]:bg-violet-50 data-[state=active]:text-violet-700">
                  <BedDouble className="h-4 w-4 mr-2" /> Room Types
                  {roomTypes.length > 0 && <Badge variant="secondary" className="ml-2 h-5 px-1.5">{roomTypes.length}</Badge>}
                </TabsTrigger>
                <TabsTrigger value="amenities" className="data-[state=active]:bg-violet-50 data-[state=active]:text-violet-700">
                  <Sparkles className="h-4 w-4 mr-2" /> Amenities
                  {amenities.length > 0 && <Badge variant="secondary" className="ml-2 h-5 px-1.5">{amenities.length}</Badge>}
                </TabsTrigger>
                <TabsTrigger value="rooms" className="data-[state=active]:bg-violet-50 data-[state=active]:text-violet-700">
                  <DoorOpen className="h-4 w-4 mr-2" /> Rooms
                  {rooms.length > 0 && <Badge variant="secondary" className="ml-2 h-5 px-1.5">{rooms.length}</Badge>}
                </TabsTrigger>
                <TabsTrigger value="rates" className="data-[state=active]:bg-violet-50 data-[state=active]:text-violet-700">
                  <CalendarDays className="h-4 w-4 mr-2" /> Rates
                  {rates.length > 0 && <Badge variant="secondary" className="ml-2 h-5 px-1.5">{rates.length}</Badge>}
                </TabsTrigger>
              </TabsList>
