...
// Water Owner Money.
//
// What the owner has put into the company, and what they have taken out.
//
// THE DISTINCTION THIS PAGE EXISTS TO MAKE
// ----------------------------------------
// A contribution is not a sale and a draw is not an expense. The company did
// not earn the first or spend the second — the owner funded it, and took funding
...
import {
  listWaterCashAccounts, listWaterOwnerMoney, getWaterOwnerMoneySummary,
  recordWaterOwnerMoney, reverseWaterOwnerMoney,
...
export default function WaterOwnerMoneyPage() {
  ...
  const [rows, setRows] = useState<WaterOwnerMoney[]>([])
  const [summary, setSummary] = useState<WaterOwnerMoneySummary | null>(null)
  ...
  const [form, setForm] = useState({
    amount: "0", waterCashAccountId: "", transactionDate: today(),
    paymentMethod: "Cash", ownerName: "", referenceNumber: "", notes: "",
  })

  const [reversing, setReversing] = useState<WaterOwnerMoney | null>(null)
  ...
  const after = isDraw
    ? balanceOf(form.waterCashAccountId) - amountNum
    : balanceOf(form.waterCashAccountId) + amountNum
  const wouldOverdraw =
    isDraw && !!form.waterCashAccountId && amountNum > 0 &&
    after < 0 && !allowsNegative(form.waterCashAccountId)
  ...
  const save = async () => {
    ...
    try {
      await recordWaterOwnerMoney({
        ...
        amount: amountNum,
        waterCashAccountId: Number(form.waterCashAccountId),
  ...
  const doReverse = async () => {
    ...
    setSaving(true)
    try {
      await reverseWaterOwnerMoney(reversing.waterOwnerMoneyId, reason.trim())
