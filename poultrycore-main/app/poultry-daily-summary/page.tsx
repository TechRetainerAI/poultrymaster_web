 ...
 *
 * Ported from the Water side (/water-reports/daily-summary): one day's income,
 * production, purchases, expenses, losses and cash position on a single
 * read-only page. Assembled client-side from existing list endpoints, so no new
 * backend surface was needed — the same trade-off the Water version makes.
 *
 * Income prefers the day's CLOSING, which is authoritative: it already merges
 * storefront and delivery-run sales without double counting, and carries the
 * cash / MoMo / bank split. With no closing yet, it falls back to the day's
 * driver returns, and says so in a footnote rather than quietly reporting a
 * smaller number as if it were the whole picture.
 *
 * PDF export goes through the browser's print pipeline; the chrome is excluded
 * with `print:hidden`.
 */
...
import {
  listPoultryDailyClosings, listPoultryLossRecords, listPoultryRawMaterialPurchases,
} from "@/lib/api/poultry-inventory"
import { listPoultryDriverReturns } from "@/lib/api/poultry-distribution"
import { getEggProductions } from "@/lib/api/egg-production"
...
export default function PoultryDailyBusinessSummaryPage() {
  ...
  async function load() {
    setBusy(true)
    try {
      // Match on the date part only — these endpoints return timestamps, and
      // some rows carry a time component while others do not.
      const onDate = (d?: string | null) => (d ?? "").slice(0, 10) === date

      // getEggProductions and getExpenses only scope to a farm when they are GIVEN
      // one — called bare they query unscoped and come back empty. The other
      // poultry pages pass getUserContext(), so this does too.
      const { farmId, userId } = getUserContext()
      if (!farmId) {
        toast({
          title: "No company selected",
          description: "Open a poultry company first — this report is scoped to one.",
          variant: "destructive",
        })
        setSummary(null)
        return
      ...
      const [closings, productionsRes, expensesRes, losses, purchases, returns] = await Promise.all([
        listPoultryDailyClosings({ fromDate: date, toDate: date }).catch(() => []),
        getEggProductions(userId, farmId).catch(() => ({ success: false, data: [] as any[] })),
        getExpenses(userId, farmId).catch(() => ({ success: false, data: [] as any[] })),
      ...
      const closing = (closings ?? []).find((c: any) => onDate(c.closingDate))

      // Production: the closing is authoritative once it exists (it is what the
      // manager signed off). Before that, fall back to the day's raw egg
      // production rows.
      const dayProductions = ((productionsRes as any)?.data ?? []).filter((p: any) => onDate(p.productionDate))
      const eggsProduced = closing?.quantityProduced ?? dayProductions.reduce((s: number, p: any) => s + (p.totalProduction ?? 0), 0)
      const brokenEggs = closing?.quantityDamaged ?? dayProductions.reduce((s: number, p: any) => s + (p.brokenEggs ?? 0), 0)

      const dayExpenses = ((expensesRes as any)?.data ?? []).filter((e: any) => onDate(e.expenseDate))
      ...
      // Only approved losses count — a pending loss record is a claim, not a cost.
      const lossesTotal = (losses ?? [])
        .filter((l: any) => l.status === "Approved")
        .reduce((s: number, l: any) => s + (l.estimatedValue ?? 0), 0)
      ...
      const income = closing
        ? {
            ...
            cash: closing.cashCollected ?? closing.cashAtHand ?? 0,
        ...
        : dayReturns.length
          ? {
              ...
              eggsSold: dayReturns.reduce((s: number, r: any) => s + (r.cratesSold ?? 0), 0),
              driverShortages: dayReturns.reduce((s: number, r: any) => s + (r.shortageAmount ?? 0), 0),
      ...
      setSummary({
        ...
        cashDifference: closing ? (closing.cashDifference ?? 0) : null,
        income,
      })
    } catch (e: any) {
              ...
              <>
                ...
                <section className="mb-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 print:gap-2">
                    ...
                    <SumCard label="Broken eggs" value={summary.brokenEggs.toLocaleString()} accent={summary.brokenEggs ? "rose" : undefined} />
                    <SumCard label="Closing egg stock" value={summary.closingStock.toLocaleString()} />
                    <SumCard label="Mortality" value={summary.mortality.toLocaleString()} accent={summary.mortality ? "rose" : undefined} />
                  </div>
                </section>

                <section className="mb-4">
                  <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-600 mb-2">Costs and cash</h2>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 print:gap-2">
                    ...
                    <SumCard
                      ...
                      value={summary.cashDifference === null ? "—" : fmtMoney(summary.cashDifference)}
                      accent={summary.cashDifference ? "rose" : undefined}
                    />
                  </div>
                </section>
                ...
                <section className="mb-4">
                  ...
                  {Object.keys(summary.expensesByCategory).length === 0 ? (
                    <p className="text-sm text-slate-500">No expenses recorded on this date.</p>
                  ) : (
                    <Table>
                      <TableHeader>
                        <TableRow>
                ...
                <footer className="text-xs text-slate-500 pt-3 border-t print:pt-2">
                  Built from the day's production, expense, purchase, loss and closing records for {date}.
