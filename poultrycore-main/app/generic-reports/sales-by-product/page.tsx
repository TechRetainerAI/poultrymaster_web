...
import { getSalesByProductReport, type GenericSalesByProductRow } from "@/lib/api/generic"
...
export default function SalesByProductPage() {
  ...
  const [rows, setRows] = useState<GenericSalesByProductRow[]>([])
  const [loading, setLoading] = useState(false)

  const load = async () => {
    setLoading(true)
    try { setRows(await getSalesByProductReport(range.fromDate, range.toDate)) }
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6">
          ...
          <h1 className="text-2xl font-semibold text-slate-900 flex items-center gap-2 mb-4">
            <ShoppingCart className="h-6 w-6 text-sky-600" /> Sales by product
          </h1>
            ...
            <Card>
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {rows.map((r, i) => (
                      <TableRow key={r.genericProductId ?? `row-${i}`}>
                        ...
                        <TableCell className="text-right text-emerald-700 font-semibold">{fmt(r.totalRevenue - r.totalCost)}</TableCell>
