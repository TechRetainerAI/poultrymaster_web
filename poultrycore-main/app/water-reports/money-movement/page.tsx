...
// Water Money Movement report.
//
// The four reports the other rail carries too, on one page rather than
...
// that prints the figures without those sentences is how a company ends up
...
import {
  listWaterCashTransfers, listWaterOwnerMoney, listWaterLoans, listWaterLoanPayments,
...
export default function WaterMoneyReportPage() {
  ...
  const [owner, setOwner] = useState<WaterOwnerMoney[]>([])
  const [loans, setLoans] = useState<WaterLoan[]>([])
  const [repayments, setRepayments] = useState<WaterLoanPayment[]>([])

  const load = async () => {
    setLoading(true)
    try {
      const [t, o, l, p] = await Promise.all([
        listWaterCashTransfers(), listWaterOwnerMoney(),
        listWaterLoans(), listWaterLoanPayments(),
      ])
        ...
        <main className="overflow-y-visible overflow-x-hidden p-4 sm:p-6 pb-16 lg:pb-4 min-w-0">
          <div className="mx-auto w-full max-w-7xl">
            ...
            <p className="text-sm text-slate-500 mb-4 max-w-3xl">
              Transfers between the company's own accounts, money the owner put in or took out, and
            ...
            {loading ? (
              ...
              <div className="space-y-6">
                ...
                <Section
                  ...
                  note="Moving money between the company's own accounts. Never company-wide money in or money out — the same money is simply in a different box."
                  tiles={[
                ...
                >
                  <Table>
                    ...
                    <TableBody>
                      ...
                      {loans.map((l) => (
                        <TableRow key={`${l.source ?? "Loan"}:${l.sourceId ?? l.waterLoanId}`}>
                ...
                <Section
                  ...
                  note="What left the bank, and what it was for. Only the interest and fee columns are an expense — the principal column is the company reducing a debt."
                  tiles={[
