...
/**
 * Cash Flow (Hotel) — an independent financial report.
 *
 * Built on GET /api/Hotel/cash-flow (migration 316), which reads:
 *   hotelpayments, hotelrestaurantorders, hoteldeposits,
 *   hotelexpenses (approved/paid), hotelpayrollruns (paid).
 *
 * It does NOT read hotelcashtransactions or hotelcashaccounts.
 * Closing cash will NOT match your account balances — that gap is what
...
export default function HotelCashFlowPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [cur, prev, all] = await Promise.allSettled([
      getCashFlow("Hotel", { fromDate: dateFrom, toDate: dateTo }),
      previousRange.from && previousRange.to
        ? getCashFlow("Hotel", { fromDate: previousRange.from, toDate: previousRange.to })
  ...
  const notes = useMemo(() => (
    <Alert className="border-slate-200 bg-slate-50 py-2">
      ...
      <AlertDescription className="text-xs text-slate-700">
        These figures come from your guest payments, restaurant orders, approved expenses and
        paid payroll — not from your cash account balances. Closing cash is <b>not</b> expected
        to match what your accounts hold; comparing the two is what reconciliation is for.
      </AlertDescription>
    </Alert>
  ), [])
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                ...
                <Tile label="Money In" value={gh(summary.moneyIn)} note="For selected period"
                      ...
                      tip="Guest payments, walk-in restaurant orders and deposits received during the selected period." />
                <Tile label="Money Out" value={gh(summary.moneyOut)} note="For selected period"
                      ...
                      tip="Approved expenses, paid payroll and refunded deposits during the selected period." />
                <Tile label="Cash at Hand" value={gh(allTime.closingCash)} note="All time"
                      tone={allTime.closingCash < 0 ? "text-rose-700" : undefined}
                      tip="Every movement ever recorded, in minus out — NOT limited to the selected period. This is what your records say you should be holding; it is not read from the accounts themselves." />
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                ...
                <Tile label="Closing Cash"
                      ...
                      tip="Opening cash plus money in minus money out. This is the period's closing figure, not all-time." />
              </div>
                ...
                <CardContent className="p-0 md:p-2">
                  {visible.length === 0 ? (
                    <p className="px-4 py-8 text-center text-sm text-slate-500">
                      No cash movement in this period. Record a guest payment, approve an expense
                      or complete a payroll run and it appears here.
                    </p>
                  ) : (
                    <MobileCardList
                      defaultOpen
                      ...
                      highlights={(r: any) => [
                        ...
                        { label: "Running cash", value: gh(r.running), accent: "amber" },
                      ]}
                      details={(r: any) => [
