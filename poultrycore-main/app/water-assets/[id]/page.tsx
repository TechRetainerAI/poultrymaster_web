...
// Asset detail — everything about one water asset in one place: what it is, what
// it cost, how it is depreciating, and where each cedi came from. The
// depreciation history reads like a statement, with the book value as at each
// row, because "what is it worth now" is the question a register exists to
// answer.
...
import {
  getWaterAsset, reverseWaterDepreciation,
  type WaterCapitalAsset,
...
export default function WaterAssetDetailPage() {
  ...
  const [asset, setAsset] = useState<WaterCapitalAsset | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try { setAsset(await getWaterAsset(id)) }
  ...
  const doReverse = async () => {
    ...
    setSaving(true)
    try {
      await reverseWaterDepreciation(reversing, reason.trim())
      toast({
  ...
  if (!asset) {
    return <div className="p-6 text-sm text-slate-600">
      Asset not found. <Link href="/water-assets" className="underline">Back to Assets</Link>
    </div>
  }
    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
          ...
          {/* The commonest question this page will be asked, answered on it. */}
          {asset.status === "Draft" && (
            <Card className="border-slate-200 bg-slate-50"><CardContent className="p-3 text-sm text-slate-600">
              {DRAFT_ASSET_NOTE}
            </CardContent></Card>
          )}
          ...
          <Card><CardContent className="p-4">
            ...
            <p className="mt-2 text-[11px] text-slate-500">{CAPITAL_NOT_EXPENSE_NOTE}</p>
          </CardContent></Card>
                    ...
                    <TableCell className="text-right">
                      {d.status === "Posted" && d.amount > 0 && (
                        <Button variant="ghost" size="sm" title="Reverse this charge"
                                onClick={() => { setReversing(d.waterAssetDepreciationId); setReason("") }}>
