...
// =============================================================================
// Assets — the water Asset Register (migrations 283, 284, 286).
//
// "Track major long-term business assets, their cost, depreciation, and current
// book value."
//
// The one thing this page exists to make true: a 600,000 borehole is money the
// company STILL OWNS, not a 600,000 hole in last month's profit. It buys with
...
//
// ONE DIFFERENCE FROM THE POULTRY REGISTER, AND IT IS DELIBERATE
// --------------------------------------------------------------
// The supplier is a DROPDOWN, not a free-text box. The poultry expense rail
// stores a supplier name on the row, so its form offers both; the water rail
// reads the name off the watersuppliers record, so a second hand-typed name
// could only contradict it. That also means a capital purchase on credit lands
// on Supplier Balances with a real supplier behind it -- which is the whole
...
import {
  listWaterCashAccounts, listWaterSuppliers,
  type WaterCashAccount, type WaterSupplier,
} from "@/lib/api/water"
import {
...
export default function WaterAssetsPage() {
  ...
  const [assets, setAssets] = useState<WaterCapitalAsset[]>([])
  const [categories, setCategories] = useState<WaterAssetCategory[]>([])
  const [summary, setSummary] = useState<WaterCapitalAssetSummary | null>(null)
  const [due, setDue] = useState<WaterAssetDepreciationDue[]>([])
  ...
  const [editing, setEditing] = useState<WaterCapitalAsset | null>(null)
  const [costFor, setCostFor] = useState<WaterCapitalAsset | null>(null)
  const [disposing, setDisposing] = useState<WaterCapitalAsset | null>(null)
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [a, c, s, d, ca, sup] = await Promise.all([
        listWaterAssets(),
        listWaterAssetCategories(),
        getWaterAssetSummary(),
        listWaterDepreciationDue().catch(() => []),
        listWaterCashAccounts().catch(() => [] as WaterCashAccount[]),
        listWaterSuppliers().catch(() => [] as WaterSupplier[]),
      ])
      setAssets(a); setCategories(c); setSummary(s); setDue(d)
      setCashAccounts((ca as WaterCashAccount[]).filter((x) => x.isActive))
      setSuppliers((sup as WaterSupplier[]).filter((x) => x.isActive && !x.isDeleted))
    } catch (e: any) {
  ...
  const filtered = useMemo(() => {
    ...
    return assets.filter((a) => {
      ...
      if (categoryFilter !== "all" && String(a.waterAssetCategoryId ?? "") !== categoryFilter) return false
      if (!q) return true
  ...
  const runDepreciation = async () => {
    setSaving(true)
    try {
      const res = await generateWaterDepreciation({})
      toast({
  ...
  if (activeFarmType && activeFarmType !== "Water") {
    return <div className="p-6 text-sm text-slate-600">
      Assets are a water company feature. <Link href="/dashboard" className="underline">Back to dashboard</Link>
    </div>
  }
    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
            ...
            extras={<>
              ...
              <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                ...
                <SelectContent>
                  ...
                  {categories.map((c) => (
                    <SelectItem key={c.waterAssetCategoryId} value={String(c.waterAssetCategoryId)}>
                      {c.categoryName}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          <AssetFormDialog
            open={newOpen} onOpenChange={setNewOpen} categories={categories}
...
/** The supplier picker, shared by the two forms that open a payable. */
function SupplierField({ value, suppliers, onChange }: {
  value: string | undefined
  suppliers: WaterSupplier[]
  onChange: (v: string) => void
}) {
  return (
    <FormField label="Supplier" hint="Needed for a credit purchase, so the balance lands on Supplier Balances.">
      <Select value={value ? String(value) : ""} onValueChange={onChange}>
        <SelectTrigger><SelectValue placeholder="No supplier" /></SelectTrigger>
        <SelectContent>
          {suppliers.map((s) => (
            <SelectItem key={s.waterSupplierId} value={String(s.waterSupplierId)}>{s.supplierName}</SelectItem>
          ))}
        </SelectContent>
      </Select>
    </FormField>
...
/**
 * Recording an asset. The cost is OPTIONAL on purpose: a borehole that will be
 * drilled starts at nothing and grows through "Add cost", which is the
 * construction workflow without a second set of screens.
 */
function AssetFormDialog({ open, onOpenChange, categories, cashAccounts, suppliers, saving, setSaving, onSaved }: {
  open: boolean; onOpenChange: (o: boolean) => void
  categories: WaterAssetCategory[]; cashAccounts: WaterCashAccount[]; suppliers: WaterSupplier[]
  ...
  const cat = categories.find((c) => String(c.waterAssetCategoryId) === String(f.assetCategoryId))
  const amount = Number(f.amount) || 0
  const credit = f.paymentMethod === "Credit"
  // Water's rule, from migration 047 and carried by 283: a Credit bill is an
  // UNPAID one, so blank means nothing has been paid rather than paid in full.
  const paid = f.amountPaid === undefined || f.amountPaid === null || f.amountPaid === ""
    ? (credit ? 0 : amount)
    : Number(f.amountPaid)
  ...
  const save = async () => {
    ...
    if (owing > 0 && !f.supplierId) {
      toast({
        title: "Choose a supplier",
        description: "There is a balance outstanding, and without a supplier it cannot appear on Supplier Balances.",
        variant: "destructive",
      })
      return
    }
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <FormSection title="What it is">
          <FormField label="Asset name">
            <Input value={f.assetName ?? ""} onChange={(e) => setF({ ...f, assetName: e.target.value })}
                   placeholder="Borehole 2" />
          </FormField>
          <FormField label="Category">
            <Select value={f.assetCategoryId ? String(f.assetCategoryId) : ""}
                    onValueChange={(v) => {
                      const c = categories.find((x) => String(x.waterAssetCategoryId) === v)
                      setF({ ...f, assetCategoryId: v, usefulLifeMonths: f.usefulLifeMonths || c?.defaultUsefulLifeMonths || "" })
                    }}>
              <SelectTrigger><SelectValue placeholder="Choose a category" /></SelectTrigger>
              <SelectContent>
                {categories.map((c) => (
                  <SelectItem key={c.waterAssetCategoryId} value={String(c.waterAssetCategoryId)}>{c.categoryName}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </FormField>
        ...
        <FormSection title="What it cost">
          ...
          <FormField label="Paid from">
            <Select value={f.cashAccountId ? String(f.cashAccountId) : ""} onValueChange={(v) => setF({ ...f, cashAccountId: v })}>
              ...
              <SelectContent>
                {cashAccounts.map((a) => (
                  <SelectItem key={a.waterCashAccountId} value={String(a.waterCashAccountId)}>{a.accountName}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </FormField>
        ...
        {amount > 0 && (
          <div className="rounded-md border border-sky-200 bg-sky-50 px-3 py-2 text-xs text-sky-900 space-y-1">
            ...
            {owing > 0 && !f.supplierId && (
              <div className="text-amber-800">
                A balance is outstanding but no supplier is chosen, so it would not appear on Supplier Balances.
              </div>
            )}
          </div>
        )}
...
function EditDialog({ asset, onClose, categories, onSaved }: {
  ...
  useEffect(() => {
    if (asset) setF({
      assetName: asset.assetName, assetCategoryId: asset.waterAssetCategoryId ?? "",
  ...
  const save = async (setFinancials: boolean) => {
    ...
    setSaving(true)
    try {
      await updateWaterAsset(asset.waterCapitalAssetId, {
        assetName: f.assetName, assetCategoryId: f.assetCategoryId ? Number(f.assetCategoryId) : null,
...
function AddCostDialog({ asset, onClose, cashAccounts, suppliers, onSaved }: {
  asset: WaterCapitalAsset | null; onClose: () => void
  cashAccounts: WaterCashAccount[]; suppliers: WaterSupplier[]; onSaved: () => Promise<void> | void
}) {
  const { toast } = useToast()
  const gh = useFmt()
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      await addWaterAssetCost(asset.waterCapitalAssetId, {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {(asset?.depreciationEntries ?? 0) > 0 && (
          <div className="flex items-start gap-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-900">
            <AlertTriangle className="w-4 h-4 mt-0.5 shrink-0" />
            <span>
              Depreciation has already been posted for this asset, so its cost cannot be changed — every month already
              charged was worked out from the current cost. Reverse the depreciation first.
            </span>
          </div>
        )}
        <div className="flex justify-end gap-2 pt-2">
...
function DisposeDialog({ asset, onClose, cashAccounts, onSaved }: {
  asset: WaterCapitalAsset | null; onClose: () => void
  cashAccounts: WaterCashAccount[]; onSaved: () => Promise<void> | void
}) {
  const { toast } = useToast()
  const gh = useFmt()
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      await disposeWaterAsset(asset.waterCapitalAssetId, {
...
function ReverseDialog({ asset, onClose, onSaved }: {
  asset: WaterCapitalAsset | null; onClose: () => void; onSaved: () => Promise<void> | void
}) {
  const { toast } = useToast()
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      await reverseWaterAsset(asset.waterCapitalAssetId, reason.trim())
