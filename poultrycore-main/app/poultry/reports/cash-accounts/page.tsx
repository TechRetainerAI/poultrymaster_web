...
/**
 * Poultry Cash Account Report — where the money sits.
 *
 * Standalone, not a registry report: there is no new backend. Same arrangement
 * as batch-production-summary and changes in this directory, and it is why this
 * page fetches for itself instead of going through PoultryReportView.
 *
 * WHY THE PERIOD FIGURES ARE COMPUTED HERE
 * ----------------------------------------
 * sppoultrycashreconciliation_getaccountstatus takes no date parameters
 * (223_PoultryCashReconciliation.postgres.sql:872) — its ledgerbalance is
...
import {
  listPoultryCashAccounts, getPoultryCashAccountCountStatus,
  listPoultryCashTransactions, listPoultryCashTransfers,
...
export default function PoultryCashAccountReportPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [accts, status, ledger, xfers] = await Promise.allSettled([
      ...
      listPoultryCashTransactions({ toDate }),
  ...
  return (
    ...
    >
      <CashAccountSections
        data={data}
        fmtMoney={fmtMoney}
        accountHref={(id) => `/poultry-cash-accounts/${id}`}
        reconcileHref="/poultry-cash-reconciliation"
        cashFlowHref="/cash-flow"
      />
    </ReportShell>
  )
}
