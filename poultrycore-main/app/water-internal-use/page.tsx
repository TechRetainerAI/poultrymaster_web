...
import {
  ...
  INTERNAL_USE_CATEGORY_LABELS, STAFF_BASED_CATEGORIES,
...
export default function WaterInternalUsePage() {
  ...
  // endpoint answers per sachet, so scale it into whichever unit is being
  ...
  useEffect(() => {
    ...
    let cancelled = false
    getWaterInternalUseSuggestedCost(form.waterProductId)
      .then((r) => {
        if (cancelled) return
        setSuggestedPerBase(r.unitCost || 0)
        setForm((p) => ({
          ...p,
          unitCost: round4((r.unitCost || 0) * (p.entryUnit === "Bag" ? sachetsPerBag : 1)),
        }))
      })
      .catch(() => { /* suggestion only — leave whatever is in the field */ })
    return () => { cancelled = true }
  }, [form.waterProductId, sachetsPerBag])

  /**
   * Switching bag ↔ sachet rescales the cost so the field keeps meaning what its
   * label says. Without this, changing the unit would silently turn "9.00 per
   * bag" into "9.00 per sachet" — a 30× error in the expense.
   */
  function changeEntryUnit(next: string) {
    setForm((p) => {
      if (p.entryUnit === next) return p
      const toBag = next === "Bag"
      const rescaled = toBag
        ? (p.unitCost || 0) * sachetsPerBag
        : (p.unitCost || 0) / Math.max(sachetsPerBag, 1)
      return { ...p, entryUnit: next, unitCost: round4(rescaled) }
    })
  }
