...
import {
  listHKSchedule, createHKSchedule, bulkCreateHKSchedule, updateHKScheduleStatus,
...
export default function HotelHKSchedulePage() {
  ...
  const [taskTypes, setTaskTypes] = useState<HotelHKTaskType[]>([])
  ...
  async function load() {
    setLoading(true)
    try {
      const [s, r, tt, st] = await Promise.all([
        listHKSchedule(schedDate),
        listHotelRooms(),
        listHotelHKTaskTypes().catch(() => []),
        listHotelStaff().catch(() => []),
      ])
      setItems(s); setRooms(r); setTaskTypes(tt)
      setStaff(st.filter((s: any) => (s.isActive ?? s.isactive) && (s.department ?? s.Department ?? "").toLowerCase().includes("housekeeping")))
    } catch (e: any) {
  ...
  async function handleBulk() {
    setBulking(true)
    try {
      const r = await bulkCreateHKSchedule({ scheduleDate: schedDate, taskType: "Daily Cleaning" })
      toast({ title: `${r.created} room(s) scheduled out of ${r.totalRooms} occupied` }); await load()
  ...
  async function handleSave() {
    if (!form.hotelRoomId) { toast({ title: "Select a room", variant: "destructive" }); return }
    if (!form.taskType) { toast({ title: "Select a task type", variant: "destructive" }); return }
    setSaving(true)
    try {
  ...
  async function doStatus(id: number, status: string) {
    try { await updateHKScheduleStatus(id, status); toast({ title: `Status → ${status}` }); await load() }
    catch (e: any) { toast({ title: "Failed", description: e?.message, variant: "destructive" }) }
  }
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((s: any, i: number) => {
                    ...
                    return (
                      <tr key={s.hotelschedid ?? i} className="border-b hover:bg-slate-50">
                        <td className="p-3 font-semibold text-violet-700">{s.roomnumber}</td>
                        <td className="p-3">{s.floorname ?? "—"}</td>
                        <td className="p-3 text-xs">{s.roomtypename ?? "—"}</td>
                        <td className="p-3"><Badge variant="outline" className="bg-violet-50 text-violet-700">{s.tasktype}</Badge></td>
                        <td className="p-3">{s.assignedto ?? "—"}</td>
                        <td className="p-3">
                          <Badge variant="outline" className={s.priority === "High" ? "bg-red-50 text-red-700" : s.priority === "Normal" ? "bg-blue-50 text-blue-700" : "bg-slate-50"}>{s.priority}</Badge>
                        </td>
                        <td className="p-3"><Badge className={STATUS_COLORS[s.status] ?? ""}>{s.status}</Badge></td>
                        <td className="p-3 text-xs">{elapsed}</td>
                        <td className="p-3 text-right whitespace-nowrap">
                          {s.status === "Scheduled" && (
                            <>
                              <Button size="sm" variant="ghost" onClick={() => doStatus(s.hotelschedid, "InProgress")} title="Start"><Play className="h-4 w-4 text-blue-600" /></Button>
                              <Button size="sm" variant="ghost" onClick={() => doStatus(s.hotelschedid, "Skipped")} title="Skip"><SkipForward className="h-4 w-4 text-slate-500" /></Button>
                            </>
                          )}
                          {s.status === "InProgress" && <Button size="sm" variant="ghost" onClick={() => doStatus(s.hotelschedid, "Completed")} title="Complete"><CheckCircle2 className="h-4 w-4 text-emerald-600" /></Button>}
                        </td>
                      </tr>
                    )
                  })}
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent>
              ...
              <div className="space-y-4">
                <div>
                  <Label>Room *</Label>
                  <Select value={form.hotelRoomId ? String(form.hotelRoomId) : "__none__"} onValueChange={(v) => setForm({ ...form, hotelRoomId: v === "__none__" ? 0 : Number(v) })}>
                    <SelectTrigger><SelectValue placeholder="Select room" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="__none__">-- Select --</SelectItem>
                      {rooms.map((r: any) => <SelectItem key={r.hotelRoomId} value={String(r.hotelRoomId)}>Room {r.roomNumber} ({r.status})</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
