...
// Nullable number → controlled-input string.
const numStr = (n: number | null | undefined) => (n == null ? "" : String(n))

function toBatchFeedLines(rows: ReturnType<typeof computeFeedLines>["rows"]): ProductionBatchUsageLine[] {
  return rows
...
function toBatchMedLines(rows: ReturnType<typeof computeMedLines>["rows"]): ProductionBatchUsageLine[] {
  return rows
    .filter((r) => r.item && r.qty > 0)
    .map((r) => ({
      itemId: r.item!.poultryRawMaterialItemId,
...
export default function EditBatchProductionRecordPage() {
  ...
  // ---- Load selectors, inventory, and the record itself --------------------
  useEffect(() => {
    (async () => {
      try {
        const { userId, farmId } = getUserContext()
        if (!userId || !farmId) { setLoading(false); return }
        const [batchRes, flockRes, items, purch, recRes] = await Promise.all([
          ...
          getBatchProductionRecord(recordId, farmId),
        ])
        ...
        if (!recRes.success || !recRes.data) {
          ...
          const r = recRes.data
          setScopeValue(
            r.batchSelectionType === "AllBatches"
              ? SCOPE_ALL
              : r.batchSelectionType === "CustomBatch"
                ? SCOPE_CUSTOM
                : String(r.selectedBirdBatchId ?? ""),
          )
          setBatchName(r.batchName ?? "")
          setSelectedFlockIds((r.includedFlocks ?? []).map((f) => f.flockId))
          setFirstCrates(r.firstPickCrates ?? 0)
          setFirstLoose(r.firstPickLooseEggs ?? 0)
          setSecondCrates(r.secondPickCrates ?? 0)
          setSecondLoose(r.secondPickLooseEggs ?? 0)
          setThirdCrates(r.thirdPickCrates ?? 0)
          setThirdLoose(r.thirdPickLooseEggs ?? 0)
          setFourthCrates(r.fourthPickCrates ?? 0)
          setFourthLoose(r.fourthPickLooseEggs ?? 0)
          setForm({
            date: (r.productionDate || "").split("T")[0] || new Date().toISOString().slice(0, 10),
            brokenEggs: numStr(r.brokenEggs),
            meatyEggs: numStr(r.meatyEggs),
            softEggs: numStr(r.softEggs),
            lostEggs: numStr(r.lostEggs),
            feedType: r.feedType ?? "",
            feedKg: numStr(r.feedKg),
            medication: r.medication ?? "",
            deaths: numStr(r.deaths),
            birdsLeft: numStr(r.birdsLeft),
            notes: r.notes ?? "",
  ...
  const handleSave = async (status: BatchStatus) => {
    try {
      ...
      let scopedFlocks: Flock[]
      if (batchSelectionType === "SpecificBatch") {
        scopedFlocks = activeFlocks.filter((f) => f.batchId === selectedBirdBatchId)
      } else if (batchSelectionType === "AllBatches") {
        scopedFlocks = activeFlocks
      } else {
        scopedFlocks = activeFlocks.filter((f) => selectedFlockIds.includes(f.flockId))
        if (scopedFlocks.length === 0) {
      ...
      const includedFlocks: ProductionBatchIncludedFlock[] = scopedFlocks.map((f) => ({
        flockId: f.flockId,
        flockName: f.name,
      ...
      const res = await updateBatchProductionRecord(recordId, input)
      if (!res.success) throw new Error(res.message || "Failed to update batch production record")
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="overflow-y-visible overflow-x-hidden p-4 sm:p-6 pb-16 lg:pb-4 min-w-0">
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-600 text-sm">Update total production for a batch or group of flocks</p>
                </div>
              </div>
              <Button
                type="button"
            ...
            {loading ? (
              <div className="text-sm text-slate-500 py-10 text-center">Loading batch production record…</div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden bg-slate-50">
                  ...
                  <div className="grid grid-cols-12 gap-4 px-4 py-4">
                    ...
                    <div className="col-span-12 md:col-span-4 space-y-2">
                      ...
                      <Input value={batchName} onChange={(e) => setBatchName(e.target.value)} placeholder="e.g. Morning collection" />
                    </div>
                ...
                <div className="flex flex-wrap gap-3 justify-end pt-2">
                  ...
                  <Button type="button" variant="outline" disabled={saving} onClick={() => handleSave("Draft")} className="min-w-[140px]">
                    {saving ? "Saving..." : "Save as Draft"}
                  </Button>
                  <Button type="submit" disabled={saving} className="min-w-[180px]">
                    {saving ? "Saving..." : "Save Batch Production"}
                  </Button>
                </div>
              </form>
            )}
