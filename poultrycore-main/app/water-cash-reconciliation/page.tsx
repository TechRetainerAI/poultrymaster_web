...
import {
  listWaterCashAccounts, getWaterCashAccountCountStatus, listWaterCashCountsForAccount,
  createWaterCashCount, updateWaterCashCount, deleteWaterCashCount,
  postWaterCashCount, reverseWaterCashCount, reconcileWaterCashBalances,
...
function WaterCashReconciliationPageInner() {
  ...
  const [counts, setCounts] = useState<WaterCashCount[]>([])
  const [accountId, setAccountId] = useState<number | null>(null)
  const [loading, setLoading] = useState(true)
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState("")
  const [reverseTarget, setReverseTarget] = useState<WaterCashCount | null>(null)
  const [discardTarget, setDiscardTarget] = useState<WaterCashCount | null>(null)
   ...
   *            part of the story.
   */
  const [editing, setEditing] = useState<{ count: WaterCashCount; mode: "draft" | "copy" } | null>(null)

  const loadAccounts = useCallback(async () => {
    setError("")
    // allSettled: the status feed comes from migration 222. If it has not been
  ...
  useEffect(() => {
    ...
    void loadAccounts().then((accs) => {
      setAccountId((current) => {
        ...
        if (wanted && accs.some((a) => a.waterCashAccountId === wanted)) return wanted
        return accs.find((a) => a.isActive)?.waterCashAccountId ?? accs[0]?.waterCashAccountId ?? null
      })
  ...
  const loadCounts = useCallback(async (id: number | null) => {
    ...
    try { setCounts(await listWaterCashCountsForAccount(id) ?? []) }
    catch { setCounts([]) }
  }, [])
  ...
  const account = useMemo(
    () => accounts.find((a) => a.waterCashAccountId === accountId) ?? null,
    [accounts, accountId],
  )
  const accountStatus = useMemo(
    () => status.find((s) => s.waterCashAccountId === accountId) ?? null,
    [status, accountId],
  )
   ...
   */
  async function postDraft(c: WaterCashCount) {
    setBusy(true)
    try {
      const res = await postWaterCashCount(c.waterCashReconciliationId)
      toast({
      ...
      if (editing?.count.waterCashReconciliationId === c.waterCashReconciliationId) { setEditing(null); setFormOpen(false) }
      await refreshAll()
    } catch (e: any) {
   ...
   * (ux_watercashrecon_one_draft), so if one exists this reopens THAT rather
  ...
  async function recalculate() {
    setBusy(true)
    try {
      await reconcileWaterCashBalances()
      toast({
                    ...
                    {openDraft && (
                      <Alert className="border-sky-200 bg-sky-50 py-2">
                        ...
                        <AlertDescription className="text-xs text-sky-900 flex flex-wrap items-center gap-2">
                          <span>
                            {openDraft.referenceNo ?? `#${openDraft.waterCashReconciliationId}`} is saved but not posted —
                          ...
                          {editing?.count.waterCashReconciliationId !== openDraft.waterCashReconciliationId && (
...
<CashCountForm
              ...
              resetKey={editing ? `${editing.mode}-${editing.count.waterCashReconciliationId}` : `new-${account.waterCashAccountId}`}
              initial={editing ? {
              ...
              disabled={!!openDraft && editing?.count.waterCashReconciliationId !== openDraft.waterCashReconciliationId}
              ...
              onSubmit={async (input) => {
                ...
                if (editing?.mode === "draft") {
                  await updateWaterCashCount(editing.count.waterCashReconciliationId, fields)
                } else {
                  await createWaterCashCount({ waterCashAccountId: account.waterCashAccountId, ...fields })
                }
                return null
              }}
      ...
      <ConfirmDeleteDialog
        ...
        onConfirm={async () => {
          ...
          await deleteWaterCashCount(discardTarget.waterCashReconciliationId)
          if (editing?.count.waterCashReconciliationId === discardTarget.waterCashReconciliationId) setEditing(null)
      ...
      <PromptDialog
        ...
        options={WATER_CASH_REVERSAL_REASONS}
        ...
        onSubmit={async (reason: string) => {
          ...
          try {
            await reverseWaterCashCount(reverseTarget.waterCashReconciliationId, reason)
...
export default function WaterCashReconciliationPage() {
  return (
    <Suspense fallback={<div className="p-6 text-slate-500">Loading...</div>}>
      <WaterCashReconciliationPageInner />
    </Suspense>
  )
}
