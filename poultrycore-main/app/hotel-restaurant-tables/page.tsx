...
import { listRestaurantTables, createRestaurantTable, type HotelRestaurantTable } from "@/lib/api/hotel"
...
export default function HotelTablesPage() {
  ...
  const [items, setItems] = useState<HotelRestaurantTable[]>([]); const [loading, setLoading] = useState(true)
  ...
  async function load() { setLoading(true); try { setItems(await listRestaurantTables()) } catch (e: any) { toast({ title: "Failed", description: e?.message, variant: "destructive" }) } finally { setLoading(false) } }
      ...
      <main className="flex-1 overflow-auto p-4 md:p-6">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent><DialogHeader><DialogTitle>Add Table</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4"><div><Label>Capacity</Label><Input type="number" min={1} value={form.capacity} onChange={(e) => setForm({...form, capacity: Number(e.target.value)})} /></div><div><Label>Location</Label><Input value={form.location} onChange={(e) => setForm({...form, location: e.target.value})} placeholder="Indoor, Patio, Bar" /></div></div>
          </div>
