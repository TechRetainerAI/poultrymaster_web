 ...
 * Internal Transfers and reconciliation status possible at all.
 *
...
import {
  listPoultryCashTransactions, listPoultryCashTransfers, getPoultryCashAccountCountStatus,
  adjustPoultryCashAccount, POULTRY_CASH_REASONS,
  type PoultryCashTransaction, type PoultryCashTransfer, type PoultryCashAccountCountStatus,
...
export default function CashFlowPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [txRes, stRes, trRes, custRes, suppRes, expRes] = await Promise.allSettled([
      listPoultryCashTransactions({ fromDate: dateFrom, toDate: dateTo }),
      getPoultryCashAccountCountStatus(),
      listPoultryCashTransfers(),
    ...
    // Expenses recorded with no cash account never reach the ledger, so Money
    // Out understates and Net Cash Flow flatters the farm. Counted exactly off
    // poultryCashAccountId rather than inferred from a total difference.
    if (expRes.status === "fulfilled" && expRes.value?.data) {
      const inRange = expRes.value.data.filter((e) => {
        const day = (e.expenseDate ?? "").split("T")[0]
        return day >= dateFrom && day <= dateTo
      })
      const missing = inRange.filter((e) => e.poultryCashAccountId == null)
      setUnlinked({
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2">
                ...
                <Tile label="Customer Balances"
                      ...
                      href="/customer-balances" icon={<Users className="h-3.5 w-3.5" />}
                ...
                <Tile label="Supplier Balances"
                      ...
                      href="/supplier-balances" icon={<Truck className="h-3.5 w-3.5" />}
              ...
              {unlinked.count > 0 && (
                <Alert className="border-amber-200 bg-amber-50 py-2">
                  ...
                  <AlertDescription className="text-xs text-amber-900">
                    ...
                    <Link href="/expenses" className="underline">Open Expenses</Link> to link{" "}
                ...
                <CardContent>
                  {accountRows.length === 0 ? (
                    <p className="py-6 text-center text-sm text-slate-500">
                      No cash accounts yet. <Link href="/poultry-cash-accounts" className="underline">Create one</Link>.
                    </p>
                  ) : (
                            ...
                            <TableRow key={a.accountId}>
                              ...
                              <TableCell className="text-right whitespace-nowrap">
                                <Button asChild size="sm" variant="ghost">
                                  <Link href={`/poultry-cash-accounts/${a.accountId}`}>View</Link>
                                </Button>
                                <Button asChild size="sm" variant="ghost">
                                  <Link href={`/poultry-cash-reconciliation?accountId=${a.accountId}`}>
                                    Reconcile
                                  </Link>
                                </Button>
                              </TableCell>
