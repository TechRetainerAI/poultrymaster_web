-- Behavioural checks for migration 291: the Loans page reads cash adjustments.
--
...
--
--   psql ... -X -c "BEGIN;" -f water-loans-read-cash-adjustments.test.sql -c "ROLLBACK;"
--
-- THE CLAIMS
...
--      balances and corrections must NOT appear: 287 owns the first two on the poultry side, and
...
BEGIN
    ...
    SELECT outstandingprincipal INTO v_before FROM spwaterloan_summary(v_farm);
    ...
    EXECUTE format($sql$
        ...
        VALUES ($1, $2, CURRENT_DATE, 'LoanReceived', 5000, 'ZZ 291 borrowed from uncle', now())
    ...
    RAISE NOTICE 'A3. loan id is 0, not the adj id expect        0  got %', v_r.waterloanid;
    ...
    RAISE NOTICE 'A5. description carried over  expect        t  got %',
        (v_r.notes = 'ZZ 291 borrowed from uncle');
    ...
    -- =====================================================================
    SELECT outstandingprincipal INTO v_after FROM spwaterloan_summary(v_farm);
    ...
    RAISE NOTICE 'B2. borrowed rose too         expect  5000.00  got %',
        ((SELECT totalborrowed FROM spwaterloan_summary(v_farm)) -
         (SELECT COALESCE(SUM(l.originalprincipal) FILTER (WHERE l.status <> 'Cancelled'), 0)
          FROM waterloans l WHERE l.farmid = v_farm));
    -- But NOT into the things an adjustment has none of.
    RAISE NOTICE 'B3. nothing repaid            expect        t  got %',
        ((SELECT totalprincipalrepaid FROM spwaterloan_summary(v_farm)) =
         (SELECT COALESCE(SUM(l.totalprincipalrepaid), 0) FROM waterloans l WHERE l.farmid = v_farm));
    RAISE NOTICE 'B4. no interest added         expect        t  got %',
        ((SELECT totalinterestpaid FROM spwaterloan_summary(v_farm)) =
         (SELECT COALESCE(SUM(l.totalinterestpaid), 0) FROM waterloans l WHERE l.farmid = v_farm));
    RAISE NOTICE 'B5. summary agrees with list  expect        t  got %',
        ((SELECT activeloans FROM spwaterloan_summary(v_farm)) =
         (SELECT COUNT(*)::int FROM spwaterloan_getall(v_farm, 'Active')));
    ...
    EXECUTE format($sql$
        INSERT INTO %s (farmid, userid, adjustmentdate, adjustmenttype, amount, description, createddate)
        VALUES ($1, $2, CURRENT_DATE, 'OwnerInjection', 9999, 'ZZ 291 owner money', now()),
               ($1, $2, CURRENT_DATE, 'Withdrawal',    -8888, 'ZZ 291 drawing',     now()),
               ($1, $2, CURRENT_DATE, 'OpeningBalance', 7777, 'ZZ 291 opening',     now()),
               ($1, $2, CURRENT_DATE, 'Correction',     6666, 'ZZ 291 fix',         now())
    $sql$, v_tbl) USING v_farm, v_user;

    SELECT COUNT(*) INTO v_n FROM spwaterloan_getall(v_farm, 'All') r
    WHERE r.source = 'CashAdjustment'
      AND r.notes IN ('ZZ 291 owner money', 'ZZ 291 drawing', 'ZZ 291 opening', 'ZZ 291 fix');
    ...
    RAISE NOTICE 'C2. outstanding is unmoved    expect  5000.00  got %',
        ((SELECT outstandingprincipal FROM spwaterloan_summary(v_farm)) - v_before);
    ...
    EXECUTE format($sql$
        INSERT INTO %s (farmid, userid, adjustmentdate, adjustmenttype, amount, description, createddate)
        VALUES ($1, $2, CURRENT_DATE, 'LoanReceived', -2000, 'ZZ 291 over-stated, corrected', now())
        RETURNING adjustmentid
    ...
    RAISE NOTICE 'D2. the correction is visible expect -2000.00  got %',
        (SELECT r.originalprincipal FROM spwaterloan_getall(v_farm, 'All') r
         WHERE r.source = 'CashAdjustment' AND r.sourceid = v_negid);

    -- =====================================================================
    -- E. REAL LOANS ARE UNAFFECTED.
    -- =====================================================================
    SELECT COUNT(*) INTO v_n FROM spwaterloan_getall(v_farm, 'All') r WHERE r.source = 'Loan';
    RAISE NOTICE 'E1. real loans still listed   expect        t  got %',
        (v_n = (SELECT COUNT(*)::int FROM waterloans l WHERE l.farmid = v_farm));
    RAISE NOTICE 'E2. and each keeps its own id expect        0  got %',
        (SELECT COUNT(*) FROM spwaterloan_getall(v_farm, 'All') r
         WHERE r.source = 'Loan' AND r.sourceid <> r.waterloanid);

    -- =====================================================================
    -- F. THE STATUS FILTER.
    -- =====================================================================
    RAISE NOTICE 'F1. legacy rows are Active    expect        2  got %',
        (SELECT COUNT(*) FROM spwaterloan_getall(v_farm, 'Active') r
         WHERE r.source = 'CashAdjustment');
    RAISE NOTICE 'F2. and absent from PaidOff   expect        0  got %',
        (SELECT COUNT(*) FROM spwaterloan_getall(v_farm, 'PaidOff') r
         WHERE r.source = 'CashAdjustment');
    RAISE NOTICE 'F3. and absent from Cancelled expect        0  got %',
        (SELECT COUNT(*) FROM spwaterloan_getall(v_farm, 'Cancelled') r
         WHERE r.source = 'CashAdjustment');

    RAISE NOTICE '--- 291 checks done. ROLL BACK this transaction. ---';
END
$t$;
