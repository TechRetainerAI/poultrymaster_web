-- Behavioural checks for migration 292: converting a Cash Flow borrowing into a
-- real, repayable loan.
--
-- Run inside a transaction you ROLL BACK; it writes adjustments and loans.
--
--   psql ... -X -c "BEGIN;" -f poultry-loan-from-cash-adjustment.test.sql -c "ROLLBACK;"
--
...
BEGIN
    ...
    EXECUTE format($sql$
        ...
        VALUES ($1, $2, CURRENT_DATE, 'LoanReceived', 4000, 'ZZ 292 borrowed from cousin', now())
    ...
    -- A. THE CONVERSION.
    -- =====================================================================
    v_loanid := sppoultryloan_fromadjustment(
    ...
    RAISE NOTICE 'A5. it has a real loan id     expect        t  got %', (v_r.poultryloanid > 0);
    RAISE NOTICE 'A6. linked back to the adjustment expect        t  got %',
        ((SELECT l.sourceadjustmentid FROM poultryloans l WHERE l.poultryloanid = v_loanid) = v_adj);
    -- No loan cash row: the adjustment is still the cash event.
    RAISE NOTICE 'A7. and NO loan cash row      expect        t  got %',
        ((SELECT l.poultrycashtransactionid FROM poultryloans l WHERE l.poultryloanid = v_loanid) IS NULL);
    ...
    RAISE NOTICE 'B1. cash accounts unmoved     expect     0.00  got %',
        ((SELECT COALESCE(SUM(a.currentbalance), 0)::numeric(14,2) FROM poultrycashaccounts a WHERE a.farmid = v_farm) - v_cash0);
    RAISE NOTICE 'B2. cash flow unmoved         expect     0.00  got %',
        ((SELECT COALESCE(SUM(r.amount), 0)::numeric(14,2) FROM sppoultrycashflow_rows(v_farm) r) - v_flow0);
    ...
    RAISE NOTICE 'C1. no longer a legacy row    expect        0  got %',
        (SELECT COUNT(*) FROM fnpoultryloan_legacy(v_farm) g WHERE g.adjustmentid = v_adj);
    RAISE NOTICE 'C2. and appears exactly once  expect        1  got %',
        (SELECT COUNT(*) FROM sppoultryloan_getall(v_farm, 'All') r
         WHERE r.originalprincipal = 4000.00 AND r.notes LIKE '%ZZ 292 borrowed from cousin%');
    ...
    ELSE
        PERFORM sppoultryloanpayment_record(
            p_farmid => v_farm, p_poultryloanid => v_loanid,
            ...
            p_notes => 'ZZ 292 first repayment', p_createdby => 'ZZ tester');

        RAISE NOTICE 'D1. the debt came down       expect  3000.00  got %',
            (SELECT l.outstandingprincipal FROM poultryloans l WHERE l.poultryloanid = v_loanid);
        RAISE NOTICE 'D2. principal repaid recorded expect  1000.00  got %',
            (SELECT l.totalprincipalrepaid FROM poultryloans l WHERE l.poultryloanid = v_loanid);
        RAISE NOTICE 'D3. interest recorded as cost expect    50.00  got %',
            (SELECT l.totalinterestpaid FROM poultryloans l WHERE l.poultryloanid = v_loanid);
        RAISE NOTICE 'D4. and the cash went out    expect -1050.00  got %',
            ((SELECT COALESCE(SUM(a.currentbalance), 0)::numeric(14,2) FROM poultrycashaccounts a WHERE a.farmid = v_farm) - v_cash0);
    END IF;

    RAISE NOTICE '--- 292 conversion checks done. ---';
END
$t$;
...
BEGIN
    ...
    EXECUTE format($sql$
        INSERT INTO %s (farmid, userid, adjustmentdate, adjustmenttype, amount, description, createddate)
        VALUES ($1, $2, CURRENT_DATE, 'LoanReceived', -1500, 'ZZ 292 correction', now())
        RETURNING adjustmentid
    $sql$, v_tbl) INTO v_neg USING v_farm, v_user;

    EXECUTE format($sql$
        INSERT INTO %s (farmid, userid, adjustmentdate, adjustmenttype, amount, description, createddate)
        VALUES ($1, $2, CURRENT_DATE, 'OwnerInjection', 3000, 'ZZ 292 owner money', now())
        RETURNING adjustmentid
    $sql$, v_tbl) INTO v_own USING v_farm, v_user;

    EXECUTE format($sql$
        INSERT INTO %s (farmid, userid, adjustmentdate, adjustmenttype, amount, description, createddate)
        VALUES ($1, $2, CURRENT_DATE, 'LoanReceived', 2200, 'ZZ 292 convert twice', now())
        RETURNING adjustmentid
    ...
    BEGIN
        PERFORM sppoultryloan_fromadjustment(v_farm, v_own, 'ZZ Lender');
    ...
    BEGIN
        PERFORM sppoultryloan_fromadjustment(v_farm, v_good, '   ');
    ...
    v_id := sppoultryloan_fromadjustment(v_farm, v_good, 'ZZ Lender');
    RAISE NOTICE 'E4. the first conversion works expect        t  got %', (v_id > 0);
    BEGIN
        PERFORM sppoultryloan_fromadjustment(v_farm, v_good, 'ZZ Lender Again');
    ...
    RAISE NOTICE '--- 292 refusal checks done. ROLL BACK this transaction. ---';
END
$e$;
