# =============================================================================
# Apply migration 252 (reversing a poultry cash transfer) to Postgres.
#
...
#
# EFFECT ON TODAY'S NUMBERS: none. Five nullable columns and two id columns on
# poultrycashtransfers, functions, and a backfill of transfernumber on the rows
# that have none -- a LABEL derived from the row's own id and date. No amount,
# no balance and no ledger row is touched.
#
# Phase 4 must print "No change to any measured total." The measure reads cash
# account balances AND the live cash-flow summary per poultry farm, because the
# claim this migration has to keep is that moving money between two boxes is
# still not money in or money out.
#
#
...
param(
    ...
    [string] $OutDir   = (Join-Path $env:TEMP 'poultry-cash-transfer-reversal'),
...
$files = @('252_PoultryCashTransferReversal.postgres.sql')

foreach ($f in $files) {
    if (-not (Test-Path (Join-Path $MigrationsDir $f))) { throw "Missing migration: $f" }
}

$checkFile = Join-Path $ChecksDir 'poultry-cash-transfer-reversal.test.sql'
...
# psql NOTICE -- of which this dry run emits about forty -- trips
...
SELECT 'poultry' AS metric, f.farmid, f.name,
       (SELECT COUNT(*) FROM poultrycashaccounts a WHERE a.farmid = f.farmid)                   AS accounts,
       (SELECT ROUND(COALESCE(SUM(a.currentbalance), 0), 2) FROM poultrycashaccounts a
         WHERE a.farmid = f.farmid)                                                             AS cash,
       (SELECT COUNT(*) FROM poultrycashtransactions t WHERE t.farmid = f.farmid)               AS ledger_rows,
       (SELECT ROUND(COALESCE(SUM(t.amount), 0), 2) FROM poultrycashtransactions t
         WHERE t.farmid = f.farmid)                                                             AS ledger_net,
       (SELECT COUNT(*) FROM poultrycashtransfers x WHERE x.farmid = f.farmid)                  AS transfers,
       (SELECT COUNT(*) FROM poultrycashtransfers x
         WHERE x.farmid = f.farmid AND x.status = 'Approved')                                   AS approved_transfers,
       (SELECT ROUND(COALESCE(s.moneyin, 0), 2)
          FROM sppoultrycashflow_summary(f.farmid, NULL, NULL) s)                               AS money_in,
       (SELECT ROUND(COALESCE(s.moneyout, 0), 2)
          FROM sppoultrycashflow_summary(f.farmid, NULL, NULL) s)                               AS money_out,
...
WHERE  f.type = 'Poultry'
  AND  EXISTS (SELECT 1 FROM poultrycashaccounts a WHERE a.farmid = f.farmid)
ORDER  BY f.farmid;
'@
...
if (-not $diff) {
    ...
    Write-Host 'Any line here is a bug -- 252 adds columns and functions, it moves no money.' -ForegroundColor Red
}

Write-Host ''
Write-Host 'RESTART THE API. /reverse is a new action and the transfer reads carry' -ForegroundColor Yellow
Write-Host 'new columns; a running PoultryFarmAPI.exe serves neither.' -ForegroundColor Yellow
