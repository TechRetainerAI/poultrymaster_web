# =============================================================================
# Apply migration 253 (poultry owner money) to Postgres.
#
...
#
# EFFECT ON TODAY'S NUMBERS: none. A new table, new functions, and ONE extra arm
# on sppoultrycashflow_rows that returns nothing until somebody records owner
# money. Every existing arm of that function is reproduced byte for byte from
# the live definition.
#
# Phase 4 must print "No change to any measured total." The measure reads the
# live cash-flow summary per poultry farm, which is exactly what the new arm
# feeds -- if it double counted the legacy cash adjustments, money_in would jump
# here and the run would stop.
#
#
...
param(
    ...
    [string] $OutDir   = (Join-Path $env:TEMP 'poultry-owner-money'),
...
$files = @('253_PoultryOwnerMoney.postgres.sql')

foreach ($f in $files) {
    if (-not (Test-Path (Join-Path $MigrationsDir $f))) { throw "Missing migration: $f" }
}

$checkFile = Join-Path $ChecksDir 'poultry-owner-money.test.sql'
...
if (-not $diff) {
    ...
    Write-Host 'Any line here is a bug -- 253 adds a table and a reader arm, it moves no money.' -ForegroundColor Red
}

Write-Host ''
Write-Host 'RESTART THE API. owner-money is a whole new controller; a running' -ForegroundColor Yellow
