# =============================================================================
# Apply migration 254 (poultry loans and repayments) to Postgres.
#
...
# arms on sppoultrycashflow_rows that return nothing until a loan exists. Every
# existing arm of that function is reproduced byte for byte from the live
# definition, which by now already carries 253's owner-money arm.
#
# Phase 4 must print "No change to any measured total." The measure reads the
# live cash-flow summary AND the P&L expense total per poultry farm -- the two
# numbers a loan repayment is most likely to corrupt if the NonCash marker on
# the interest and fee expenses were ever dropped.
#
#
...
param(
    ...
    [string] $OutDir   = (Join-Path $env:TEMP 'poultry-loans'),
...
$files = @('254_PoultryLoans.postgres.sql')

foreach ($f in $files) {
    if (-not (Test-Path (Join-Path $MigrationsDir $f))) { throw "Missing migration: $f" }
}

$checkFile = Join-Path $ChecksDir 'poultry-loans.test.sql'
...
SELECT 'poultry' AS metric, f.farmid, f.name,
       ...
       (SELECT ROUND(COALESCE(p.totalexpenses, 0), 2)
          FROM sppoultryreport_profitloss(f.farmid, '2000-01-01'::date, '2099-12-31'::date) p)  AS pnl_expenses
FROM   farms f
...
if (-not $diff) {
    ...
    Write-Host 'Any line here is a bug -- 254 adds tables and reader arms, it moves no money.' -ForegroundColor Red
}

Write-Host ''
Write-Host 'RESTART THE API. loans is a whole new controller; a running' -ForegroundColor Yellow
Write-Host 'PoultryFarmAPI.exe will 404 every one of its routes.' -ForegroundColor Yellow
