# =============================================================================
# Apply migration 250 (the Generic subscription dashboard and reports) to
# Postgres.
#
...
#
# EFFECT ON TODAY'S NUMBERS: none, and this one is stronger than 242-249. That
# set added columns and tables; 250 adds nothing but FUNCTIONS. No table, no
# column, no row. Phase 4 must print "No change to any measured total." There is
# no expected-to-move line, with no exceptions.
#
# The dry run seeds a Generic company with subscriptions, invoices, expenses,
# staff and a payroll run, asserts about sixty figures against it, and throws
# the lot away. Read the "expect X got Y" lines: every one must agree.
#
...
param(
    ...
    [string] $OutDir   = (Join-Path $env:TEMP 'generic-subscription-reporting'),
...
$files = @('250_GenericSubscriptionReporting.postgres.sql')

foreach ($f in $files) {
    if (-not (Test-Path (Join-Path $MigrationsDir $f))) { throw "Missing migration: $f" }
}

$checkFile = Join-Path $ChecksDir 'generic-subscription-reporting.test.sql'
...
# psql NOTICE -- of which this dry run emits about sixty -- trips
...
Write-Host ''
Write-Host '=== DIFF (expected: identical -- 250 adds functions and nothing else) ===' -ForegroundColor Cyan
...
if (-not $diff) {
    ...
    Write-Host 'Any line here is a bug -- 250 writes nothing.' -ForegroundColor Red
}

Write-Host ''
Write-Host 'RESTART THE API. The new endpoints live in a new controller; a running' -ForegroundColor Yellow
Write-Host 'PoultryFarmAPI.exe is serving the old assembly and will 404 them.' -ForegroundColor Yellow
