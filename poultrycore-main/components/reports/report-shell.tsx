...
export function ReportShell({
  ...
  // Render the "Generated" timestamp only after mount. Computing `new Date()`
  // during render makes the server and client HTML differ (the clock moves
  // between SSR and hydration), which trips React's hydration check.
  ...
  async function sendEmail() {
    const opts = resolvePdf()
    if (!opts) return
    // Accept several addresses, comma / semicolon / newline separated.
    ...
    setSending(true)
    try {
      const res = await emailTableAsPdf(withBrandMeta(opts), { to: recipients.join(",") })
      if (res.success) {
        const sentTo = recipients.length === 1 ? recipients[0] : `${recipients.length} recipients`
        toast({ title: "Report emailed", description: `Sent to ${sentTo}.` })
        setEmailOpen(false)
      } else {
        ...
        <DialogContent className="print:hidden">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-500">
              Separate multiple addresses with commas. A PDF of this report (current
              date range and filters) will be generated and sent to each recipient.
            </p>
          </div>
          <DialogFooter>
