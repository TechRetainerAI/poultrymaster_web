...
import {
  Egg, Package, Bird, TrendingUp, TrendingDown, Wallet, ReceiptText,
  Filter, Search, RotateCcw, ArrowLeft,
} from "lucide-react"
...
export function usePoultryDashboardData() {
  ...
  return {
    ...
    birdsLeftLatestTotal, totalFeedKg, prodDaily, prodByTime,
    // financial
...
// -----------------------------------------------------------------------------
// Section renderers — one per dashboard view. Each takes the shared data.
// -----------------------------------------------------------------------------
function ProductionSection({ data }: { data: PoultryDashboardData }) {
  const {
    totalEggs, totalCrates, looseEggs, avgDaily, totalMortality, totalFeedKg,
    birdsLeftLatestTotal, selectedFlock, prodDaily, prodByTime,
  } = data
  return (
    <>
      {/* Metrics Row 1 */}
...
function FinancialSection({ data }: { data: PoultryDashboardData }) {
  const {
...
function DailySection({ data }: { data: PoultryDashboardData }) {
  const {
...
function InsightsSection({ data }: { data: PoultryDashboardData }) {
...
/** Renders the cards/charts/tables for a single dashboard view. */
export function PoultryDashboardSection({ view, data }: { view: DashboardView; data: PoultryDashboardData }) {
  if (view === "production") return <ProductionSection data={data} />
  if (view === "financial") return <FinancialSection data={data} />
  if (view === "daily") return <DailySection data={data} />
...
export function PoultryDashboardView({ view }: { view: DashboardView }) {
  ...
  const meta = DASHBOARD_VIEW_META[view]
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="overflow-y-visible overflow-x-hidden p-4 sm:p-6 pb-16 min-w-0">
          ...
          <div className="space-y-4">
            ...
            <PoultryDashboardSection view={view} data={data} />
          </div>
        </main>
      </div>
    </div>
