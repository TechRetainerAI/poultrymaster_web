...
// What a payroll deduction actually IS (migration 314, spec sections 30-32).
//
...
import {
  listWaterPayrollDeductions, saveWaterPayrollDeduction,
  deleteWaterPayrollDeduction, listEligibleWaterEmployeeLoans,
  PAYROLL_DEDUCTION_TYPES, PAYROLL_DEDUCTION_TYPE_LABELS,
  type WaterPayrollDeduction, type WaterEmployeeLoanEligible,
} from "@/lib/api/water"
...
export function PayrollDeductionsDialog({
  ...
  const [rows, setRows] = useState<WaterPayrollDeduction[]>([])
  const [eligible, setEligible] = useState<WaterEmployeeLoanEligible[]>([])
  ...
  const chosen = eligible.find((e) => String(e.waterEmployeeLoanId) === loanId)
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [ded, elig] = await Promise.all([
        listWaterPayrollDeductions(target.waterPayrollItemId),
        listEligibleWaterEmployeeLoans(target.waterStaffId),
      ])
      setRows(ded)
      setEligible(elig)
      ...
      if (elig.length > 0) {
        setLoanId(String(elig[0].waterEmployeeLoanId))
            ...
            <Table>
              ...
              <TableBody>
                {rows.length === 0 ? (
                  ...
                  <TableRow key={r.waterPayrollItemDeductionId ?? `legacy-${i}`}
                    ...
                    <TableCell className="text-right">
                      ...
                      {editable && !r.isLegacy && r.status === "Draft" && r.waterPayrollItemDeductionId && (
                        <Button size="sm" variant="ghost" disabled={busy}
                                onClick={() => void run(
                                  () => deleteWaterPayrollDeduction(r.waterPayrollItemDeductionId!),
                                  "Deduction removed")}>
                  ...
                  {isLoanType && (
                    <div className="space-y-1">
                      ...
                      <Select value={loanId} onValueChange={(v) => {
                        setLoanId(v)
                        const e = eligible.find((x) => String(x.waterEmployeeLoanId) === v)
                        ...
                        <SelectContent>
                          ...
                          {eligible.map((e) => (
                            <SelectItem key={e.waterEmployeeLoanId} value={String(e.waterEmployeeLoanId)}>
                ...
                <div className="flex justify-end">
                  <Button disabled={busy || !canAdd} onClick={() => void run(async () => {
                    await saveWaterPayrollDeduction({
                      waterPayrollItemId: target.waterPayrollItemId,
                      deductionType: type,
                      amount,
                      waterEmployeeLoanId: isLoanType ? Number(loanId) : null,
