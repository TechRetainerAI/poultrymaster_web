...
import {
  recalculateWaterRawMaterialStock,
  type WaterRawMaterialItem, type WaterRawMaterialRecalcRow,
} from "@/lib/api/water"

// Reusable "Recalculate stock" control for Water raw materials & supplies.
// Recomputes CurrentQuantity from purchases and usage — for every item, or a
// single picked one. Drop it on any Water inventory page.
export function WaterRecalculateStockButton({
  items, onDone, variant = "outline", size, className, label = "Recalculate stock",
}: {
  items: WaterRawMaterialItem[]
  ...
  const [result, setResult] = useState<WaterRawMaterialRecalcRow[] | null>(null)
  ...
  async function run() {
    ...
    try {
      ...
      const rows = await recalculateWaterRawMaterialStock(itemId)
  ...
  return (
    <>
      <Button variant={variant} size={size} className={className}
        ...
        title="Recompute raw-material stock from purchases and usage">
          ...
          {result === null ? (
            <div className="space-y-3">
              <p className="text-sm text-slate-600">
                ...
                <span className="font-medium"> total purchased (in production units) − used in production</span>,
              ...
              <div>
                ...
                <Select value={target} onValueChange={setTarget}>
                  ...
                  <SelectContent>
                    ...
                    {active.map((i) => (
                      <SelectItem key={i.waterRawMaterialItemId} value={String(i.waterRawMaterialItemId)}>
