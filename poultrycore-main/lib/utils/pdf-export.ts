...
export interface PdfExportOptions {
  ...
  /**
   * What one row IS, plural — "stock movements", "sales", "employees".
   * The letterhead says "Showing: all 73 stock movements" rather than the bare
   * "Records: 73" it used to print. Defaults to "records".
   */
  recordLabel?: string
  /**
   * How many rows there were BEFORE the on-screen filters narrowed them, so the
   * letterhead can say "73 of 412" — the number a reader actually needs to know
   * whether they are holding the whole picture or a slice of it.
   */
...
/**
 * The letterhead's scope line: what this report actually covers.
 *
 * It replaces a bare "Records: 73", which told the reader the size of the file
 * and nothing about its scope — 73 out of how many, narrowed by what? Reading
 * the three cases:
 *
 *   Showing:  73 of 412 stock movements  (filters cut it down)
 *   Showing:  all 412 stock movements    (nothing was filtered out)
 *   Showing:  73 stock movements         (no total was supplied to compare to)
 */
function scopeLine(
  shown: number,
  ...
  const noun = recordLabel?.trim() || "records"
  const n = shown.toLocaleString()
  if (total != null && total > shown) return `Showing:  ${n} of ${total.toLocaleString()} ${noun}`
  if (total != null || !hasFilters) return `Showing:  all ${n} ${noun}`
  return `Showing:  ${n} ${noun}`
}
...
function drawFilterChips(
  ...
  // Nothing filtered, nothing to draw. A heading over a chip reading "None —
  // all records included" spent two lines and a section title saying less than
  // the scope line above it ("Showing: all 412 stock movements") already does.
  if (filters.length === 0) return yStart
  ...
  const items: PdfExportFilter[] = filters
...
async function buildPdfDoc(opts: PdfExportOptions) {
  ...
  meta.push(scopeLine(opts.rows.length, opts.totalBeforeFilters, opts.recordLabel, (opts.filtersUsed?.length ?? 0) > 0))
...
export interface MultiTablePdfOptions {
  ...
  currencyLabel?: string
  filtersUsed?: PdfExportFilter[]
  /** See PdfExportOptions.recordLabel. */
  recordLabel?: string
  /** See PdfExportOptions.totalBeforeFilters. */
  totalBeforeFilters?: number
  summaryCards?: PdfExportSummaryCard[]
...
async function buildMultiTablePdfDoc(opts: MultiTablePdfOptions) {
  ...
  if (opts.fromDate && opts.toDate) meta.push(`Period:  ${opts.fromDate}  to  ${opts.toDate}`)
  if (opts.currencyLabel) meta.push(`Currency:  ${opts.currencyLabel}`)
  meta.push(scopeLine(totalRecords, opts.totalBeforeFilters, opts.recordLabel, (opts.filtersUsed?.length ?? 0) > 0))
  if (opts.generatedBy) meta.push(`Generated by:  ${opts.generatedBy}`)
