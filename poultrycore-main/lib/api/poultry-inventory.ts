...
// ---------------------------------------------------------------------------
// Deferred inventory costs (migration 288).
//
// The read side of cost recognition: which purchases still hold cost that has
// not reached Profit & Loss, which consumptions moved it there, and how any one
// consumption's cost was worked out.
//
...
// a second answer that immediately disagreed with the P&L.
// ---------------------------------------------------------------------------
 ...
 */
export type DeferredCostScope = "DEFERRED" | "RECOGNIZED" | "EXCEPTION" | "ALL"

export interface PoultryDeferredPurchase {
  poultryRawMaterialPurchaseId: number
  purchaseDate: string
  poultryRawMaterialItemId: number
  itemName?: string | null
  ...
  operationalCost: number
  /** What this lot deferred when it was created. */
  deferredTotalCost: number
  ...
  /** Gap between the two counts. Non-zero ⇒ Exception. */
  ...
  sourceFeedProductionBatchId?: number | null
  feedProductionBatchNumber?: string | null
  /** Produced by feed production rather than bought — has a batch, not a supplier. */
...
export interface PoultryDeferredRecognition {
  poultryRawMaterialUsageId: number
  usedDate: string
...
export interface PoultryConsumptionCostLayer {
  poultryRawMaterialUsageId: number
  poultryRawMaterialItemId: number
  itemName?: string | null
  category?: string | null
...
export interface PoultryDeferredCostSummary {
...
export interface PoultryDeferredCostResponse {
  summary: PoultryDeferredCostSummary
  purchases: PoultryDeferredPurchase[]
}

export interface DeferredCostFilters {
  scope?: DeferredCostScope
  itemId?: number
  supplierId?: number
  category?: string
  fromDate?: string
...
export const getPoultryDeferredCosts = (f?: DeferredCostFilters) => {
  const qs = new URLSearchParams({ farmId: activeFarmId() })
  ...
  return jget<PoultryDeferredCostResponse>(`/Poultry/deferred-inventory-costs?${qs.toString()}`)
}

/** Every usage that drew on one purchase lot. Empty ⇒ nothing has drawn yet. */
export const getPoultryDeferredCostHistory = (purchaseId: number) =>
  jget<PoultryDeferredRecognition[]>(
    `/Poultry/deferred-inventory-costs/${purchaseId}/history?farmId=${encodeURIComponent(activeFarmId())}`)

/**
 * How one production record's consumption cost was arrived at, lot by lot.
 * Keyed on the production record rather than the usage row because editing a
 * record rewrites its usage rows — the same reason migration 266 links its
 * expense that way.
 */
export const getPoultryConsumptionCostBreakdown = (productionRecordId: number) =>
  jget<PoultryConsumptionCostLayer[]>(
    `/Poultry/deferred-inventory-costs/breakdown/${productionRecordId}?farmId=${encodeURIComponent(activeFarmId())}`)
