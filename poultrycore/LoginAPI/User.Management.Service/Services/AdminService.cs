...
namespace User.Management.Service.Services
{
    public class AdminService : IAdminService
    {
        ...
        public async Task<List<FarmSummary>> GetFarmsAsync()
        {
            try
            {
                // Farms table can be empty or behind while AspNetUsers already has FarmId/FarmName from registration.
                // Merge distinct farm IDs from dbo.Farms and dbo.AspNetUsers; prefer Farms.Name, else any user FarmName.
                var dalFarms = await _subscriptionDal.GetFarmsAsync();

                var users = await _userManager.Users
                    .AsNoTracking()
                    .Where(u => !string.IsNullOrWhiteSpace(u.FarmId))
                    .Select(u => new { u.FarmId, u.FarmName, u.IsStaff, u.IsSubscriber })
                    .ToListAsync();

                static string NormFarmId(string id) => id.Trim();

                var farmIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                foreach (var f in dalFarms)
                {
                    if (string.IsNullOrWhiteSpace(f.FarmId)) continue;
                    farmIds.Add(NormFarmId(f.FarmId));
                }

                foreach (var u in users)
                    farmIds.Add(NormFarmId(u.FarmId));

                var dalByFarmId = dalFarms
                    .Where(f => !string.IsNullOrWhiteSpace(f.FarmId))
                    .GroupBy(f => NormFarmId(f.FarmId), StringComparer.OrdinalIgnoreCase)
                    .ToDictionary(g => g.Key, g => g.First(), StringComparer.OrdinalIgnoreCase);

                var usersNorm = users
                    .Select(u => new
                    {
                        FarmId = NormFarmId(u.FarmId),
                        u.FarmName,
                        u.IsStaff,
                        u.IsSubscriber,
                    })
                ...
                var results = new List<FarmSummary>(farmIds.Count);
                foreach (var farmId in farmIds)
                {
                    string? farmName = null;
                    if (dalByFarmId.TryGetValue(farmId, out var row) && !string.IsNullOrWhiteSpace(row.Name))
                        farmName = row.Name;

                    if (string.IsNullOrWhiteSpace(farmName))
                    {
                        farmName = usersNorm
                            .Where(x => string.Equals(x.FarmId, farmId, StringComparison.OrdinalIgnoreCase))
                            .Select(x => x.FarmName)
                    ...
                    results.Add(new FarmSummary
                    {
                        ...
                        StaffCount = people.Count(p => p.IsStaff),
                        HasPaidSubscription = people.Any(p => p.IsSubscriber),
                    });
                }
        ...
        public async Task<int> GetFarmCountAsync()
        {
            try
            {
                return (await GetFarmsAsync()).Count;
            }
            catch (Exception ex)
            {
