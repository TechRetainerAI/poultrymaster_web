...
export default function ExpensesPage() {
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    if (!createForm.category) { setCreateError("Please select a category"); toast({ title: "Validation error", description: "Please select a category.", variant: "destructive" }); setCreateLoading(false); return }
    if (!createForm.description.trim()) { setCreateError("Please enter a description"); toast({ title: "Validation error", description: "Description is required.", variant: "destructive" }); setCreateLoading(false); return }
    if (!createForm.amount || Number(createForm.amount) <= 0) { setCreateError("Please enter a valid amount"); toast({ title: "Validation error", description: "Amount is required and must be greater than 0.", variant: "destructive" }); setCreateLoading(false); return }
    if (!createForm.paymentMethod) { setCreateError("Please select a payment method"); toast({ title: "Validation error", description: "Please select a payment method.", variant: "destructive" }); setCreateLoading(false); return }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (!editForm.category) { setEditError("Please select a category"); toast({ title: "Validation error", description: "Please select a category.", variant: "destructive" }); setEditLoading(false); return }
    if (!editForm.description.trim()) { setEditError("Please enter a description"); toast({ title: "Validation error", description: "Description is required.", variant: "destructive" }); setEditLoading(false); return }
    if (!editForm.amount || Number(editForm.amount) <= 0) { setEditError("Please enter a valid amount"); toast({ title: "Validation error", description: "Amount is required and must be greater than 0.", variant: "destructive" }); setEditLoading(false); return }
    if (!editForm.paymentMethod) { setEditError("Please select a payment method"); toast({ title: "Validation error", description: "Please select a payment method.", variant: "destructive" }); setEditLoading(false); return }
                  ...
                  <SheetContent side="bottom" className="h-[75vh] rounded-t-2xl">
                    ...
                    <div className="space-y-4 overflow-y-auto pb-8">
                      ...
                      <div className="flex gap-2 pt-4">
                        <Button variant="outline" className="flex-1" onClick={() => { setSearchQuery(""); setFromDate(""); setToDate(""); setSelectedFlock("all"); setSelectedMonth("all"); setSelectedCategory("all") }}>Clear all</Button>
                        <Button className="flex-1" onClick={() => setFiltersOpen(false)}>Apply</Button>
                      </div>
                      <div className="flex gap-2">
                        <Button variant="outline" className="flex-1" onClick={handleExportPDF}><FileTextIcon className="w-4 h-4 mr-2" />PDF</Button>
                        <Button variant="outline" className="flex-1" onClick={handleExportCSV}><Download className="w-4 h-4 mr-2" />CSV</Button>
                      </div>
                    </div>
                  </SheetContent>
                </Sheet>
