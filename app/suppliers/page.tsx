...
export default function SuppliersPage() {
  ...
  useEffect(() => {
    if (!permissions.isLoading && !canAccessSuppliersPage(permissions.featureAccess, permissions.isAdmin)) {
  ...
  useEffect(() => {
    ...
    if (!canAccessSuppliersPage(permissions.featureAccess, permissions.isAdmin)) return
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    if (!createForm.name.trim() || !createForm.contactPhone.trim() || !createForm.city.trim() || !createForm.address.trim()) {
      ...
      setCreateError(msg)
      toastFormGuide(toast, "Add the supplier name, phone, city, and address — those fields keep purchases and deliveries accurate.")
      setCreateLoading(false)
      return
    }

    const supplier: SupplierInput = { farmId, userId, ...createForm }
  ...
  const openEditDialog = async (supplierId: number) => {
    ...
    const result = await getSupplier(supplierId, userId, farmId)
    if (result.success && result.data) {
      const s = result.data
      setEditForm({
        name: s.name || "",
      ...
      setEditError(result.message || "Failed to load supplier")
    }
    setEditFetching(false)
  }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    const supplier: SupplierInput = { farmId, userId, ...editForm }
    const result = await updateSupplier(editingSupplierId, supplier)
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    const result = await deleteSupplier(deletingId, userId, farmId)
    if (result.success) {
  ...
  const sortedSuppliers = useMemo(() => sortData(filteredSuppliers, sortKey, sortDir), [filteredSuppliers, sortKey, sortDir])
    ...
    <div className="flex min-h-screen bg-slate-50">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-start gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-slate-600">Manage vendors you buy feed and supplies from</p>
                </div>
              </div>
              <Button className="gap-2 w-full sm:w-auto h-11 sm:h-10 bg-amber-700 hover:bg-amber-800 shrink-0" onClick={openCreateDialog}>
            ...
            {loading ? (
              ...
              <Card className="bg-white">
                <CardContent className="py-12 text-center">
                  ...
                  {!searchQuery && (
                    <Button className="gap-2 bg-amber-700 hover:bg-amber-800" onClick={openCreateDialog}>
                              ...
                              <div className="flex items-start justify-between gap-3 cursor-pointer">
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="mt-1 flex items-center gap-2 text-sm text-slate-600">
                                    {s.contactEmail && <span className="truncate">{s.contactEmail}</span>}
                                    {s.city && <span className="text-slate-400">•</span>}
                              ...
                              <div className="mt-4 pt-4 border-t border-slate-100 space-y-2 text-sm">
                                <div className="grid grid-cols-2 gap-2">
                                  <div>
                                    ...
                                    <span className="font-medium">{s.contactPhone || "—"}</span>
                                  </div>
                                  <div>
                                    <span className="text-slate-500">Address</span>{" "}
                                    <span className="font-medium truncate block">{s.address || "—"}</span>
                                  </div>
                                </div>
                                <div className="flex gap-2 pt-2">
                                  <Button
                                    ...
                                    onClick={() => openEditDialog(s.supplierId)}
                                  >
                                  ...
                                  {permissions.canDelete && (
                                    <Button
                                      ...
                                      onClick={() => openDeleteDialog(s.supplierId)}
                                    >
                        ...
                        <TableHeader>
                          <TableRow className="border-b">
                            <SortableHeader
                              ...
                              onSort={handleSort}
                              className={cn("font-semibold text-slate-900 min-w-[120px]", isMobile && "sticky-col-date bg-slate-50")}
                            />
                            <SortableHeader
                              label="Email"
                              ...
                              currentDirection={sortDir}
                              onSort={handleSort}
                              className="font-semibold text-slate-900 min-w-[200px] hidden sm:table-cell"
                            />
                            <SortableHeader
                              label="Phone"
                              ...
                              currentDirection={sortDir}
                              onSort={handleSort}
                              className="font-semibold text-slate-900 min-w-[150px] hidden md:table-cell"
                            />
                            <SortableHeader
                              label="City"
                              sortKey="city"
                              ...
                              currentDirection={sortDir}
                              onSort={handleSort}
                              className="font-semibold text-slate-900 min-w-[120px] hidden lg:table-cell"
                            />
                            <SortableHeader
                              label="Address"
                              sortKey="address"
                              ...
                              currentDirection={sortDir}
                              onSort={handleSort}
                              className="font-semibold text-slate-900 min-w-[200px] hidden xl:table-cell"
                            />
                            <SortableHeader
                              label="Date added"
                              sortKey="createdDate"
                              ...
                              currentDirection={sortDir}
                              onSort={handleSort}
                              className="font-semibold text-slate-900 min-w-[120px] hidden xl:table-cell"
                            />
                            <TableHead
                              className={cn("font-semibold text-slate-900 text-center min-w-[100px]", isMobile && "sticky-col-actions bg-slate-50")}
                            >
                              Actions
                            </TableHead>
                          </TableRow>
                            ...
                            <TableRow key={s.supplierId} className="hover:bg-slate-50 transition-colors">
                              ...
                              <TableCell className="text-slate-600 hidden sm:table-cell">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="truncate max-w-[200px]">{s.contactEmail}</span>
                                </div>
                              </TableCell>
                              ...
                              <TableCell className={cn("text-center whitespace-nowrap bg-white", isMobile && "sticky-col-actions")}>
                                <div className="flex items-center justify-center gap-1 min-w-[90px]">
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                    className="h-8 w-8 hover:bg-amber-50 hover:text-amber-800"
                                    onClick={() => openEditDialog(s.supplierId)}
                                  >
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
                <div className="flex items-center gap-4">
                  <span className="text-sm text-slate-600">
                    Showing {startIndex + 1} to {Math.min(endIndex, sortedSuppliers.length)} of {sortedSuppliers.length}{" "}
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Truck className="w-5 h-5 text-amber-700" /> Add new supplier
            </DialogTitle>
            <DialogDescription>Enter the supplier information below</DialogDescription>
          </DialogHeader>
          {createError && (
            <Alert variant="destructive">
          ...
          <form onSubmit={handleCreateSubmit} className="space-y-4">
            <div className="rounded-xl border border-slate-200 overflow-hidden">
              <div className="bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">Contact</div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-white">
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-slate-700">Business / name *</Label>
                  <Input name="name" placeholder="e.g. Agro Feed Ltd" value={createForm.name} onChange={handleCreateChange} required disabled={createLoading} />
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-slate-700">Phone *</Label>
                  <Input name="contactPhone" type="tel" placeholder="+233 …" value={createForm.contactPhone} onChange={handleCreateChange} required disabled={createLoading} />
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-slate-700">Email</Label>
                  <Input name="contactEmail" type="text" placeholder="optional" value={createForm.contactEmail} onChange={handleCreateChange} disabled={createLoading} />
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-slate-700">City *</Label>
                  <Input name="city" placeholder="City" value={createForm.city} onChange={handleCreateChange} required disabled={createLoading} />
                </div>
              </div>
            </div>
            <div className="rounded-xl border border-slate-200 overflow-hidden">
              ...
              <div className="p-4 bg-white">
                <div className="space-y-2">
                  ...
                  <Input name="address" placeholder="Street, region" value={createForm.address} onChange={handleCreateChange} required disabled={createLoading} />
                </div>
              </div>
            </div>
            <div className="flex gap-3 justify-end pt-2">
              <Button type="button" onClick={() => setIsCreateDialogOpen(false)} className="bg-red-600 hover:bg-red-700 text-white">
                Cancel
              </Button>
              <Button type="submit" disabled={createLoading} className="bg-amber-700 hover:bg-amber-800">
                {createLoading ? (
                  <>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Pencil className="w-5 h-5 text-amber-700" /> Edit supplier
            </DialogTitle>
          ...
          {editFetching ? (
            <div className="py-8 text-center">
              ...
              <p className="text-slate-600">Loading supplier…</p>
            </div>
          ) : (
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div className="rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-white">
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-slate-700">Phone *</Label>
                    <Input name="contactPhone" type="tel" value={editForm.contactPhone} onChange={handleEditChange} required disabled={editLoading} />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-slate-700">Email</Label>
                    <Input name="contactEmail" type="text" value={editForm.contactEmail} onChange={handleEditChange} disabled={editLoading} />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-slate-700">City *</Label>
                    <Input name="city" value={editForm.city} onChange={handleEditChange} required disabled={editLoading} />
                  </div>
                </div>
              </div>
              <div className="rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="p-4 bg-white">
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-slate-700">Full address *</Label>
                    <Input name="address" value={editForm.address} onChange={handleEditChange} required disabled={editLoading} />
                  </div>
                </div>
              </div>
              <div className="flex gap-3 justify-end pt-2">
                <Button type="button" onClick={() => setIsEditDialogOpen(false)} className="bg-red-600 hover:bg-red-700 text-white">
                  Cancel
                </Button>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this supplier? Inventory rows that reference this supplier may need to be updated separately.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
