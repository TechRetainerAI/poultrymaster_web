...
namespace User.Management.API.Controllers
{
	...
	public class PaymentsController : ControllerBase
	{
		...
		public IActionResult SubscriptionTiers()
		{
			...
			var (a1, a2, a3) = FarmSubscriptionPricing.ResolveTierMonthlyAmounts(o);
			return Ok(new
			{
				...
				tiers = new object[]
				{
					new { id = "tier1", maxBirds = o.Tier1MaxBirds, monthlyAmount = a1, label = $"0–{o.Tier1MaxBirds:N0} birds" },
					new { id = "tier2", maxBirds = o.Tier2MaxBirds, monthlyAmount = a2, label = $"{o.Tier1MaxBirds + 1:N0}–{o.Tier2MaxBirds:N0} birds" },
					new { id = "tier3", maxBirds = (int?)null, monthlyAmount = a3, label = $"Over {o.Tier2MaxBirds:N0} birds" },
				}
			});
		}
		...
		public async Task<IActionResult> InitializePaystack([FromBody] CreateCheckoutSessionRequest req)
		{
			...
			if (string.IsNullOrWhiteSpace(_paystackSettings.SecretKey))
				return BadRequest(
					"Paystack secret key is not configured on this API. On Cloud Run set env PAYSTACK_SECRET_KEY or PaystackSettings__SecretKey on the Login API service (not the Next.js site).");
			...
			// JWT may expose email as ClaimTypes.Email, long URI, or short "email" (when MapInboundClaims is false).
			...
			if (string.IsNullOrWhiteSpace(email))
			{
				return BadRequest(new ErrorResponse
				{
					...
					{
						Message =
							"Unable to determine payer email. Sign in again so your session includes an email claim, or ensure the Authorization Bearer token is sent with this request.",
					},
				});
			}
