-- =============================================================================
-- Migration 148: Production records support MULTIPLE feed lines
-- =============================================================================
-- Mirror of migration 147 (multi-medication) for the feed side. A production
-- record can now draw N feed items from the raw-material inventory, each with its
-- own FIFO/LIFO/HIFO batch cost, instead of a single SpecificFeedUsedId.
--
-- NOTE: this is the inventory-based "Feed used" costing only. The separate
-- manual "Feed Type" + "Feed (kg)" fields and their FeedUsage sync are unchanged.
--
-- Design (identical shape to 147):
--   * New child table dbo.ProductionRecordFeeds — one snapshot row per line.
--   * spPoultryProductionRawMaterialSync gains @FeedsJson
--     ([{ "itemId": <int>, "qty": <decimal> }, ...]); it applies EACH line through
--     spPoultryRawMaterialItem_ConsumeBatches, writing a snapshot row per line.
--     When @FeedsJson is NULL it falls back to the legacy single @FeedItemId/
--     @FeedQty pair (Delete/reverse-only paths keep working unchanged).
--   * The aggregate feed columns on ProductionRecords are still populated (sum of
--     quantities, weighted-average unit cost, sum of cost, first line's id/name)
--     so existing reports/closing keep working — the new UI reads the lines.
--
...
-- -----------------------------------------------------------------------------
-- 1. Child table: one row per feed line on a production record.
-- -----------------------------------------------------------------------------
IF OBJECT_ID('dbo.ProductionRecordFeeds', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.ProductionRecordFeeds (
        ProductionRecordFeedId   INT IDENTITY(1,1) PRIMARY KEY,
        ...
        UnitCost                 DECIMAL(14,4) NOT NULL CONSTRAINT DF_ProdRecFeed_UnitCost DEFAULT (0),
        TotalCost                DECIMAL(14,2) NOT NULL CONSTRAINT DF_ProdRecFeed_TotalCost DEFAULT (0),
        CreatedAt                DATETIME2     NOT NULL CONSTRAINT DF_ProdRecFeed_CreatedAt DEFAULT (SYSUTCDATETIME()),
        CONSTRAINT FK_ProdRecFeed_ProductionRecord FOREIGN KEY (ProductionRecordId)
            REFERENCES dbo.ProductionRecords (Id) ON DELETE CASCADE
    );
    CREATE INDEX IX_ProdRecFeed_Record ON dbo.ProductionRecordFeeds (FarmId, ProductionRecordId);
END
GO

-- -----------------------------------------------------------------------------
-- 2. Batch-aware production sync — feed AND medication multi-line.
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    --    the feed + medication snapshot lines so they can be re-derived.
    UPDATE p
...
-- -----------------------------------------------------------------------------
-- 3. Insert — accept @FeedsJson + @MedicationsJson, derive aggregate columns.
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    -- Derive the aggregate feed columns from the multi-line JSON when given.
    DECLARE @FeedAgg TABLE (Seq INT IDENTITY(1,1), ItemId INT, Qty DECIMAL(14,3));
    IF (@FeedsJson IS NOT NULL AND LEN(@FeedsJson) > 0)
        INSERT INTO @FeedAgg (ItemId, Qty)
        SELECT ItemId, Qty FROM OPENJSON(@FeedsJson) WITH (ItemId INT '$.itemId', Qty DECIMAL(14,3) '$.qty')
        WHERE ItemId IS NOT NULL AND ISNULL(Qty, 0) > 0;

    IF (@FeedsJson IS NOT NULL)
    BEGIN
        IF EXISTS (SELECT 1 FROM @FeedAgg)
        BEGIN
            SET @TotalFeedConsumed = (SELECT SUM(Qty) FROM @FeedAgg);
            SELECT TOP 1 @SpecificFeedUsedId = ItemId FROM @FeedAgg ORDER BY Seq;
            SET @SpecificFeedUsedName = (SELECT ItemName FROM dbo.PoultryRawMaterialItems WHERE PoultryRawMaterialItemId = @SpecificFeedUsedId AND FarmId = @FarmId);
        END
        ELSE
        BEGIN
            SET @SpecificFeedUsedId = NULL; SET @SpecificFeedUsedName = NULL; SET @TotalFeedConsumed = NULL;
        END
    END
...
-- -----------------------------------------------------------------------------
-- 5. Get procs — return the feed AND medication lines as JSON array columns.
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.ProductionRecordFeeds TO [Techretainer];
    PRINT '148: granted EXECUTE/table rights on multi-feed objects to Techretainer.';
END
GO

PRINT '148_ProductionRecordMultiFeed.sql complete.';
GO
