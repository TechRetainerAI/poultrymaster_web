-- =============================================================================
-- 291_WaterLoansReadCashAdjustments.postgres.sql
--
-- Purpose
-- -------
...
-- Cash Flow pages.
--
-- The water twin of 290. That file did this for poultry, mirroring 287, which
-- did it for Owner Money and deliberately left LoanReceived alone: "borrowed
-- money, not owner money. The Loans module owns it." This pair is what makes
...
-- different tables:
--
--   /water-cash-flow   POST /Cash/Adjustment -> cashadjustment,
--                      adjustmenttype 'LoanReceived'
--   /water-loans       spwaterloan_create -> waterloans
--
-- Cash Flow reads BOTH, so the borrowing shows up there. The Loans page reads
-- only waterloans, so the same borrowing is missing from the page whose entire
...
--
-- Cash Flow is NOT touched. Copying cashadjustment rows into waterloans would
...
-- spwaterloanpayment_record path that can reach it, so its outstanding balance
...
--
--   OwnerInjection / Withdrawal   owner money, not borrowing. 287 owns those on
--                                 the poultry side, and counting them here would
--                                 state the same money twice across two pages.
...
-- silently do nothing.
--
-- waterloanid is 0 on a legacy row. It is deliberately NOT reused to carry the
...
-- 287 and 290 do this, and the same reason the gotcha is worth repeating.
-- -----------------------------------------------------------------------------
DO $drop$
DECLARE
    r record;
BEGIN
    FOR r IN
        ...
        WHERE  n.nspname = 'public'
          AND  p.proname IN ('spwaterloan_getall',
                             'spwaterloan_summary',
                             'fnwaterloan_legacy')
    LOOP
...
-- environments and `cashadjustments` on others -- 235, 236, 287 and 290 all do the
...
-- 2. The list.
--
-- Reproduced from 259 with the legacy arm unioned on and two columns added:
--
...
--
-- Every real-loan column is byte for byte what 259 returned.
-- -----------------------------------------------------------------------------
...
AS $function$
    ...
    -- 291. The legacy arm. Everything a loan record knows and an adjustment does
    ...
    SELECT 0,                                   -- NOT the adjustment id. See the header.
           ...
           NULL::integer,                       -- watercashaccountid
    ...
    FROM   public.fnwaterloan_legacy(p_farmid) g
    ...
    ORDER  BY 14 DESC, 1 DESC;   -- startdate, then id: 259's order, kept
$function$;

COMMENT ON FUNCTION public.spwaterloan_getall(text, text) IS
...
-- 3. Summary.
--
-- Reproduced from 259. The legacy rows are added to what was borrowed, what was
...
AS $function$
    ...
    legacy AS (
        -- 291. Counted into the debt, per the header. A negative correction nets
...
COMMENT ON FUNCTION public.spwaterloan_summary(text) IS
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'poultryapp') THEN
        ...
        GRANT EXECUTE ON FUNCTION public.spwaterloan_getall(text, text) TO poultryapp;
        GRANT EXECUTE ON FUNCTION public.spwaterloan_summary(text) TO poultryapp;
    END IF;
END
$grants$;
...
BEGIN
    ...
    IF v_farm IS NULL THEN
        RAISE NOTICE '291: no water company to verify against.';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO v_list FROM spwaterloan_getall(v_farm, 'All');
    SELECT activeloans INTO v_sum FROM spwaterloan_summary(v_farm);
    RAISE NOTICE '291: list returned % rows, summary counts % active.', v_list, v_sum;

    -- Exactly one source value per row, and legacy rows never carry a real id.
    IF EXISTS (SELECT 1 FROM spwaterloan_getall(v_farm, 'All') r
               WHERE r.source NOT IN ('Loan', 'CashAdjustment')) THEN
        RAISE EXCEPTION '291: a row came back with an unknown source.';
    END IF;
    IF EXISTS (SELECT 1 FROM spwaterloan_getall(v_farm, 'All') r
               WHERE r.source = 'CashAdjustment' AND r.waterloanid <> 0) THEN
        RAISE EXCEPTION '291: a legacy row carried a real loan id. The page would offer actions against the wrong record.';
    END IF;
    RAISE NOTICE '291: every row is tagged, and no legacy row carries a loan id.';
END
$verify$;
