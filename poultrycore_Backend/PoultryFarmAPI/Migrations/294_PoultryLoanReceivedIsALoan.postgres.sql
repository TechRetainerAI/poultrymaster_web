-- =============================================================================
-- 294_PoultryLoanReceivedIsALoan.postgres.sql
--
-- Purpose
-- -------
-- A "Loan received" IS a loan. Stop making the user say so twice.
--
-- WHAT WAS WRONG WITH 292
-- =======================
-- 290 surfaced Cash Flow borrowings on the Loans page and counted them as debt.
-- 292 gave them a repayment path -- behind a button labelled "Make it a loan".
--
...
-- ==============================================
-- poultryloans.lendername was NOT NULL, which is right for a loan somebody typed
...
-- =========================
-- None, and this is the same argument 292 made. A backfilled loan carries NO
-- cash row: the adjustment it came from is still the cash event, 292's guard
...
-- over-stated borrowing, not a borrowing -- poultryloans has
-- CHECK (originalprincipal > 0) and would reject it anyway. Those stay as
-- adjustments, 290's legacy read keeps netting them off against the debt, and
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE  n.nspname = 'public' AND p.proname = 'sppoultryloan_fromadjustment'
    ) THEN
        RAISE EXCEPTION '294 requires 292 (sppoultryloan_fromadjustment is missing).';
    END IF;
END
$guard$;
...
-- -----------------------------------------------------------------------------
ALTER TABLE poultryloans ALTER COLUMN lendername DROP NOT NULL;

COMMENT ON COLUMN poultryloans.lendername IS
...
--
-- Reproduced from 292 with one refusal relaxed. Everything else -- the
...
DECLARE
    ...
    v_id     integer;
BEGIN
    -- 294. No longer refused. A blank lender is how a BACKFILLED row says "this
...
BEGIN
    v_tbl := COALESCE(to_regclass('public.cashadjustment')::text,
                      to_regclass('public.cashadjustments')::text);
    IF v_tbl IS NULL THEN
        RAISE NOTICE '294: no cashadjustment table -- nothing to backfill.';
        RETURN;
    END IF;
    ...
    $sql$, v_tbl)
    LOOP
        PERFORM sppoultryloan_fromadjustment(
            p_farmid       => r.farmid,
            p_adjustmentid => r.adjustmentid,
            p_lendername   => NULL,        -- 294. Not known, and not invented.
            p_createdby    => r.userid);
        v_n := v_n + 1;
    END LOOP;

    RAISE NOTICE '294: backfilled % Cash Flow borrowing(s) into loans.', v_n;
END
$backfill$;
...
SELECT 'a loan may now have no lender' AS check,
       CASE WHEN (SELECT is_nullable FROM information_schema.columns
                  WHERE table_name = 'poultryloans' AND column_name = 'lendername') = 'YES'
...
SELECT 'every backfilled loan is repayable',
       CASE WHEN COUNT(*) FILTER (
                WHERE l.poultryloanid <= 0 OR l.status <> 'Active'
                   OR l.outstandingprincipal <= 0) = 0
            THEN 'OK' ELSE 'SOME ARE NOT' END
FROM   poultryloans l WHERE l.sourceadjustmentid IS NOT NULL
