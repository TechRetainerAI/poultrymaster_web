-- =============================================================================
-- 222_WaterCashReconciliation.postgres.sql
--
-- Purpose
-- -------
...
--
-- Ported from the Generic module
-- ------------------------------
-- Migration 111 already built this for Generic: GenericCashReconciliations plus
-- spGenericCashReconciliation_Create/_GetByAccount. Water had none of it. This
-- is a faithful port so the two modules stay recognisable, plus the two things
-- 111 lacks: reversal, and per-transaction clearing status.
--
-- One deliberate difference from 111. It does:
--     UPDATE GenericCashAccounts SET CurrentBalance = @ActualBalance
-- Water forbids that. currentbalance moves only by "± amount" alongside a ledger
-- row, and migration 048's spWaterCashAccount_ReconcileBalance would overwrite a
-- directly-set value on its next run anyway. We post the DIFFERENCE and let the
-- balance follow -- arithmetically identical, concurrency-safe, and it survives
-- a later recalculation because the correction lives in the ledger.
--
-- "Reconcile" is already taken, twice
-- -----------------------------------
--   spWaterCashAccount_ReconcileBalance (048)  = recompute the cached balance
--   POST /Water/cash-accounts/reconcile-balances = the same thing over the API
-- Neither is a cash count. This migration therefore names its table
-- watercashreconciliations but keeps the ROUTE and the UI wording distinct
-- ("Cash count" vs "Recalculate balances"). Do not merge the two ideas.
--
-- Additive on function surface, by necessity
-- ------------------------------------------
-- Migration 221's header records that the Postgres bodies of the pre-211 water
-- SPs are not in this repo (001-210 are T-SQL kept as the logic of record; the
-- live functions were converted elsewhere). CREATE OR REPLACE cannot change a
-- RETURNS TABLE column list, so altering spwatercashtransaction_getbyfarm would
-- mean DROP + rewriting a body nobody here can read.
--
-- So: NEW FUNCTIONS ONLY, ZERO DROPS. The clearing columns are exposed through a
-- new spwatercashtransaction_getledger rather than by editing the existing read.
--
-- THE DAILY-CLOSING TRAP -- read before extending this
-- ----------------------------------------------------
-- fnwaterdailyclosing_livetotals derives expected cash at hand from sales,
-- payments, driver returns, production, raw-material purchases and EXPENSES. All
-- nine migrations that define or redefine it (058, 097, 100, 102, 104, 105, 107,
-- 108, 109) were checked: none of them reference watercashtransactions or
-- watercashaccounts. So a cash-account adjustment CANNOT distort daily closing,
-- and migration 212's carve-out technique is not needed here.
--
-- The corollary is the danger. The leak would be through waterexpenses, which
-- daily closing DOES read. "Book the bank charge as an expense" is a natural
-- follow-up request, and acting on it would silently understate expected cash
-- every single day -- exactly the bug migration 212 had to carve out for
-- internal use. THIS FEATURE MUST NEVER WRITE A waterexpenses ROW. The reason
-- field carries the categorisation instead.
--
-- Also note waterdailyclosings already has cashathand/actualcashcounted/
...
-- -----------------------------------------------------------------------------
ALTER TABLE watercashaccounts ADD COLUMN IF NOT EXISTS lastreconciledat     timestamp     NULL;
ALTER TABLE watercashaccounts ADD COLUMN IF NOT EXISTS lastreconciledbalance numeric(14,2) NULL;

-- -----------------------------------------------------------------------------
-- 2. The cash count header.
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS watercashreconciliations (
    watercashreconciliationid serial PRIMARY KEY,
    farmid                    text          NOT NULL,
    -- ON DELETE CASCADE deliberately. spWaterCashAccount_Delete hard-deletes an
    -- account that has no transactions -- which is exactly an account somebody
    -- opened a draft count against and then wanted gone. Without the cascade
    -- that path dies on a raw FK error and the account becomes undeletable. A
    -- POSTED count always implies transactions, so that path deactivates the
    -- account instead and nothing cascades.
    watercashaccountid        integer       NOT NULL
        REFERENCES watercashaccounts (watercashaccountid) ON DELETE CASCADE,
    referenceno               text          NULL,
    reconciliationdate        timestamp     NOT NULL DEFAULT (now() at time zone 'utc'),

    -- What the LEDGER says: openingbalance + SUM(amount). Note Water's identity
    ...
    -- What watercashaccounts.currentbalance claimed at post time. Equal to
    ...
    adjustmenttransactionid   integer       NULL
        REFERENCES watercashtransactions (watercashtransactionid),
...
CREATE INDEX IF NOT EXISTS ix_watercashrecon_farm_account
    ON watercashreconciliations (farmid, watercashaccountid, reconciliationdate DESC);
CREATE INDEX IF NOT EXISTS ix_watercashrecon_farm_status
    ON watercashreconciliations (farmid, status);
CREATE UNIQUE INDEX IF NOT EXISTS ux_watercashrecon_farm_ref
    ON watercashreconciliations (farmid, referenceno) WHERE referenceno IS NOT NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_watercashrecon_one_draft
    ON watercashreconciliations (farmid, watercashaccountid) WHERE status = 'Draft';
...
-- action; see spwatercashtransaction_clearbefore in section 5.
ALTER TABLE watercashtransactions
...
ALTER TABLE watercashtransactions
    ADD COLUMN IF NOT EXISTS watercashreconciliationid integer       NULL;
ALTER TABLE watercashtransactions
    ADD COLUMN IF NOT EXISTS clearingnotes             text          NULL;

DO $fk$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_watercashtxn_reconciliation') THEN
        ALTER TABLE watercashtransactions
            ADD CONSTRAINT fk_watercashtxn_reconciliation
            FOREIGN KEY (watercashreconciliationid)
            REFERENCES watercashreconciliations (watercashreconciliationid)
            ON DELETE SET NULL;
    END IF;
END;
$fk$;

CREATE INDEX IF NOT EXISTS ix_watercashtxn_uncleared
    ON watercashtransactions (farmid, watercashaccountid, transactiondate)
    WHERE clearingstatus <> 'Cleared';
CREATE INDEX IF NOT EXISTS ix_watercashtxn_reconciliation
    ON watercashtransactions (watercashreconciliationid)
    WHERE watercashreconciliationid IS NOT NULL;
...
-- is handled everywhere else in this schema (see 047's note).

-- -----------------------------------------------------------------------------
-- 4. Draft lifecycle: insert / update / delete.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.spwatercashreconciliation_insert(
    p_farmid             text,
...
BEGIN
    ...
    -- Belt to ux_watercashrecon_one_draft's braces, so the user gets a sentence
    -- rather than a unique-violation.
    SELECT h.watercashreconciliationid INTO v_open
    FROM   watercashreconciliations h
    WHERE  h.farmid = p_farmid AND h.watercashaccountid = p_watercashaccountid
      AND  h.status = 'Draft'
    LIMIT  1;
    ...
    WHERE  t.farmid = p_farmid AND t.watercashaccountid = p_watercashaccountid;

    v_when := COALESCE(p_reconciliationdate, now() at time zone 'utc');

    INSERT INTO watercashreconciliations
        (farmid, watercashaccountid, reconciliationdate, systembalance,
         actualbalance, difference, reason, notes, status, createdby)
    VALUES
        (p_farmid, p_watercashaccountid, v_when, v_system,
         p_actualbalance,
         CASE WHEN p_actualbalance IS NULL THEN 0 ELSE ROUND(p_actualbalance - v_system, 2) END,
         p_reason, p_notes, 'Draft', p_createdby)
    RETURNING watercashreconciliationid INTO v_id;

    -- Human-quotable reference, same shape as spwaterinternalusage_insert.
    UPDATE watercashreconciliations
    SET    referenceno = 'CC-' || to_char(v_when, 'YYYY') || '-' || lpad(v_id::text, 4, '0')
    WHERE  watercashreconciliationid = v_id;

    RETURN v_id;
END;
$function$;

CREATE OR REPLACE FUNCTION public.spwatercashreconciliation_update(
    p_watercashreconciliationid integer,
    p_farmid             text,
    p_reconciliationdate timestamp DEFAULT NULL,
    p_actualbalance      numeric   DEFAULT NULL,
    p_reason             text      DEFAULT NULL,
...
DECLARE
    ...
    v_system  numeric(14,2);
BEGIN
    SELECT h.status, h.watercashaccountid INTO v_status, v_account
    FROM   watercashreconciliations h
    WHERE  h.watercashreconciliationid = p_watercashreconciliationid AND h.farmid = p_farmid;

    IF v_status IS NULL THEN
        RAISE EXCEPTION 'Cash count % not found.', p_watercashreconciliationid;
    END IF;
    ...
    -- really there.
    SELECT a.openingbalance + COALESCE((SELECT SUM(t.amount) FROM watercashtransactions t
                                        WHERE t.farmid = p_farmid AND t.watercashaccountid = v_account), 0)
      INTO v_system
    FROM   watercashaccounts a
    WHERE  a.watercashaccountid = v_account AND a.farmid = p_farmid;
    ...
    WHERE  watercashreconciliationid = p_watercashreconciliationid AND farmid = p_farmid;
END;
$function$;

CREATE OR REPLACE FUNCTION public.spwatercashreconciliation_delete(
    p_watercashreconciliationid integer, p_farmid text, p_userid text DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
AS $function$
DECLARE
    v_status text;
BEGIN
    SELECT h.status INTO v_status FROM watercashreconciliations h
    WHERE  h.watercashreconciliationid = p_watercashreconciliationid AND h.farmid = p_farmid;
    ...
    DELETE FROM watercashreconciliations
    WHERE watercashreconciliationid = p_watercashreconciliationid AND farmid = p_farmid;
END;
$function$;
...
-- 5. Clearing status.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.spwatercashtransaction_setclearing(
    p_farmid             text,
    p_watercashaccountid integer,
...
BEGIN
    ...
    PERFORM 1 FROM watercashaccounts
    WHERE  watercashaccountid = p_watercashaccountid AND farmid = p_farmid;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Cash account not found.';
    END IF;
    ...
    SELECT COALESCE(h.referenceno, '#' || h.watercashreconciliationid::text) INTO v_locked
    FROM   watercashtransactions t
    JOIN   watercashreconciliations h
           ON h.watercashreconciliationid = t.watercashreconciliationid
    WHERE  t.farmid = p_farmid
      AND  t.watercashtransactionid IN (
    ...
    UPDATE watercashtransactions t
    SET    clearingstatus = p_clearingstatus,
           ...
           watercashreconciliationid = NULL,
           clearingnotes  = NULLIF(btrim(COALESCE(p_clearingnotes, '')), '')
    WHERE  t.farmid = p_farmid
      AND  t.watercashaccountid = p_watercashaccountid
      AND  t.watercashtransactionid IN (
               SELECT (json_array_elements_text(p_transactionidsjson::json))::integer);
...
CREATE OR REPLACE FUNCTION public.spwatercashtransaction_clearbefore(
    p_farmid text, p_watercashaccountid integer, p_cutoff timestamp,
...
BEGIN
    ...
    WHERE  farmid = p_farmid
      AND  watercashaccountid = p_watercashaccountid
...
-- 6. Post.
-- -----------------------------------------------------------------------------
-- Inlines its own ledger insert rather than calling spwatercashaccount_adjust.
-- That function writes SourceId = NULL (migration 112), so the delta rule below
...
CREATE OR REPLACE FUNCTION public.spwatercashreconciliation_post(
    p_watercashreconciliationid integer,
    p_farmid                    text,
...
BEGIN
    SELECT h.status, h.watercashaccountid, h.actualbalance, h.reconciliationdate,
    ...
    FROM   watercashaccounts a
    WHERE  a.watercashaccountid = v_accountid AND a.farmid = p_farmid
    FOR    UPDATE;
    ...
    -- and migration 048's ReconcileBalance exists precisely because it drifts.
    ...
    WHERE  t.farmid = p_farmid
      ...
      AND  t.sourceid   = p_watercashreconciliationid;
    ...
    IF v_delta <> 0 THEN
        ...
        INSERT INTO watercashtransactions
            (farmid, watercashaccountid, transactiondate, transactiontype,
             ...
             clearingstatus, cleareddate, clearedby, watercashreconciliationid)
        VALUES
            (p_farmid, v_accountid, COALESCE(v_when, now() at time zone 'utc'), v_type,
             'ReconciliationAdjustment', p_watercashreconciliationid,
             v_delta, v_true + v_delta,
             'Cash count ' || COALESCE(v_ref, '#' || p_watercashreconciliationid::text)
             ...
             'Cleared', (now() at time zone 'utc'), p_postedby, p_watercashreconciliationid)
        RETURNING watercashtransactionid INTO v_adjid;
       ...
       AND btrim(p_clearedtransactionidsjson) NOT IN ('', '[]') THEN
        WITH ids AS (
            ...
            SET    clearingstatus = 'Cleared',
                   ...
                   watercashreconciliationid = p_watercashreconciliationid
            FROM   ids
            WHERE  t.watercashtransactionid = ids.id
              AND  t.farmid = p_farmid
              AND  t.watercashaccountid = v_accountid
...
-- 7. Reverse.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.spwatercashreconciliation_reverse(
    p_watercashreconciliationid integer,
    p_farmid     text,
    p_reason     text DEFAULT NULL,
...
BEGIN
    SELECT h.status, h.watercashaccountid, h.referenceno
    ...
    IF v_delta <> 0 THEN
        INSERT INTO watercashtransactions
            (farmid, watercashaccountid, transactiondate, transactiontype,
             sourcetype, sourceid, amount, balanceaftertransaction, description,
             createdby, approvedby, approvedat, createdat,
             clearingstatus, cleareddate, clearedby, watercashreconciliationid, clearingnotes)
        SELECT p_farmid, v_accountid, (now() at time zone 'utc'),
               ...
               'Reversal of cash count ' || COALESCE(v_ref, '#' || p_watercashreconciliationid::text)
                   || COALESCE(': ' || NULLIF(btrim(p_reason), ''), ''),
               p_reversedby, p_reversedby, (now() at time zone 'utc'), (now() at time zone 'utc'),
               'Cleared', (now() at time zone 'utc'), p_reversedby,
               p_watercashreconciliationid, 'Nets the posted correction to zero.'
        FROM   watercashaccounts a
        WHERE  a.watercashaccountid = v_accountid AND a.farmid = p_farmid
        RETURNING watercashtransactionid INTO v_revid;
    ...
    SET    clearingstatus = 'Uncleared',
           ...
           clearingnotes = COALESCE(NULLIF(clearingnotes, '') || ' | ', '')
                           ...
                           || COALESCE(v_ref, '#' || p_watercashreconciliationid::text)
                           || ' was reversed.'
    WHERE  farmid = p_farmid
      AND  watercashaccountid = v_accountid
      AND  watercashreconciliationid = p_watercashreconciliationid
    ...
    FROM   watercashreconciliations h
    WHERE  h.farmid = p_farmid AND h.watercashaccountid = v_accountid AND h.status = 'Posted'
    ORDER  BY h.reconciliationdate DESC, h.watercashreconciliationid DESC
    LIMIT  1;
...
-- 8. Reads.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.spwatercashreconciliation_getall(
    p_farmid             text,
...
RETURNS TABLE (
    watercashreconciliationid integer, farmid text, watercashaccountid integer,
...
STABLE
AS $$
    SELECT h.watercashreconciliationid, h.farmid, h.watercashaccountid,
    ...
    FROM   watercashreconciliations h
    JOIN   watercashaccounts a ON a.watercashaccountid = h.watercashaccountid
    WHERE  h.farmid = p_farmid
      AND  (p_watercashaccountid IS NULL OR h.watercashaccountid = p_watercashaccountid)
...
CREATE OR REPLACE FUNCTION public.spwatercashreconciliation_getbyid(
    p_watercashreconciliationid integer, p_farmid text)
RETURNS TABLE (
    watercashreconciliationid integer, farmid text, watercashaccountid integer,
    accountname text, accounttype text, referenceno text, reconciliationdate timestamp,
    systembalance numeric, systembalancecached numeric, actualbalance numeric,
...
AS $$
    ...
    -- watercashreconciliationid, so an unqualified WHERE would be ambiguous.
    SELECT r.* FROM public.spwatercashreconciliation_getall(p_farmid) r
    WHERE  r.watercashreconciliationid = p_watercashreconciliationid;
$$;

CREATE OR REPLACE FUNCTION public.spwatercashreconciliation_getbyaccount(
    p_watercashaccountid integer, p_farmid text)
RETURNS TABLE (
    watercashreconciliationid integer, farmid text, watercashaccountid integer,
    accountname text, accounttype text, referenceno text, reconciliationdate timestamp,
    systembalance numeric, systembalancecached numeric, actualbalance numeric,
...
STABLE
AS $$
    SELECT * FROM public.spwatercashreconciliation_getall(p_farmid, p_watercashaccountid);
$$;

-- The ledger read WITH clearing. A new function rather than an edit to
-- spwatercashtransaction_getbyfarm, because that function's Postgres body is not
-- in this repo and CREATE OR REPLACE cannot change a RETURNS TABLE column list.
-- The first 15 columns match it exactly (048:222) so callers can move over
-- without remapping.
CREATE OR REPLACE FUNCTION public.spwatercashtransaction_getledger(
    p_farmid                    text,
    p_watercashaccountid        integer   DEFAULT NULL,
    p_fromdate                  timestamp DEFAULT NULL,
    p_todate                    timestamp DEFAULT NULL,
    p_clearingstatus            text      DEFAULT NULL,
    p_watercashreconciliationid integer   DEFAULT NULL)
RETURNS TABLE (
    watercashtransactionid integer, farmid text, watercashaccountid integer,
    ...
    watercashreconciliationid integer, clearingnotes text,
...
AS $$
    SELECT t.watercashtransactionid, t.farmid, t.watercashaccountid, a.accountname,
           ...
           t.watercashreconciliationid, t.clearingnotes,
           h.referenceno
    FROM   watercashtransactions t
    JOIN   watercashaccounts a ON a.watercashaccountid = t.watercashaccountid
    LEFT   JOIN watercashreconciliations h
           ON h.watercashreconciliationid = t.watercashreconciliationid
    WHERE  t.farmid = p_farmid
      AND  (p_watercashaccountid IS NULL OR t.watercashaccountid = p_watercashaccountid)
      ...
      AND  (p_watercashreconciliationid IS NULL
            OR t.watercashreconciliationid = p_watercashreconciliationid)
...
-- spwatercashaccount_getall stays untouched.
CREATE OR REPLACE FUNCTION public.spwatercashreconciliation_getaccountstatus(p_farmid text)
RETURNS TABLE (
    watercashaccountid integer, accountname text, accounttype text, isactive boolean,
...
STABLE
AS $$
    SELECT a.watercashaccountid, a.accountname, a.accounttype, a.isactive,
           a.currentbalance,
           ...
           d.watercashreconciliationid
    FROM   watercashaccounts a
    LEFT   JOIN LATERAL (
        SELECT SUM(t.amount) AS total
        FROM   watercashtransactions t
        WHERE  t.farmid = a.farmid AND t.watercashaccountid = a.watercashaccountid
    ) l ON TRUE
    LEFT   JOIN LATERAL (
    ...
    LEFT   JOIN LATERAL (
        SELECT h.watercashreconciliationid
        FROM   watercashreconciliations h
        WHERE  h.farmid = a.farmid AND h.watercashaccountid = a.watercashaccountid
          AND  h.status = 'Draft'
        LIMIT  1
    ) d ON TRUE
...
BEGIN
    IF to_regclass('public.iampermissions') IS NULL THEN
        RAISE NOTICE '222: iampermissions not present, skipping catalog seed.';
        RETURN;
    END IF;
...
WHERE  n.nspname = 'public'
  AND  p.proname IN ('spwatercashreconciliation_insert','spwatercashreconciliation_update',
                     'spwatercashreconciliation_delete','spwatercashreconciliation_post',
                     'spwatercashreconciliation_reverse','spwatercashreconciliation_getall',
                     'spwatercashreconciliation_getbyid','spwatercashreconciliation_getbyaccount',
                     'spwatercashreconciliation_getaccountstatus','spwatercashtransaction_getledger',
                     'spwatercashtransaction_setclearing','spwatercashtransaction_clearbefore')
UNION ALL
SELECT 'no NULL clearing status',
       CASE WHEN NOT EXISTS (SELECT 1 FROM watercashtransactions WHERE clearingstatus IS NULL)
            THEN 'OK' ELSE 'BAD' END
UNION ALL
-- Pre-existing drift is expected and is NOT a failure of this migration; _post
-- heals the account it touches. Run POST /Water/cash-accounts/reconcile-balances
-- once per farm before rollout to clear it everywhere.
SELECT 'balance identity (drift is pre-existing)',
       CASE WHEN NOT EXISTS (
              SELECT 1 FROM watercashaccounts a
              WHERE  a.currentbalance <> a.openingbalance
                   + COALESCE((SELECT SUM(t.amount) FROM watercashtransactions t
                               WHERE t.watercashaccountid = a.watercashaccountid
                                 AND t.farmid = a.farmid), 0))
            THEN 'OK' ELSE 'DRIFT PRESENT - run reconcile-balances' END;

-- Which accounts have drifted, if any.
SELECT a.farmid, a.watercashaccountid, a.accountname,
       a.currentbalance AS cached,
       a.openingbalance + COALESCE((SELECT SUM(t.amount) FROM watercashtransactions t
                                    WHERE t.watercashaccountid = a.watercashaccountid
