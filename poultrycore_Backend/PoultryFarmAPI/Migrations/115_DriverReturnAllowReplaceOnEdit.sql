/* ============================================================================
   115_DriverReturnAllowReplaceOnEdit.sql

   Keep the "one open return per delivery" guard (migration 114) but let the
   EDIT / reverse-and-re-record flow through: that flow inserts the corrected
   return and then deletes the old one, so the guard must ignore the return it
   is replacing. New @ReplaceReturnId param: when set, that return id is
   excluded from the duplicate check. NULL = a brand-new return (full check).
   Idempotent.
   ============================================================================ */
...
CREATE OR ALTER PROCEDURE dbo.spWaterDriverReturn_Insert
    ...
    -- When editing/replacing a return, the id being replaced is excluded from
    -- the one-open-return-per-delivery check.
    @ReplaceReturnId INT = NULL
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    IF EXISTS (SELECT 1 FROM dbo.WaterDriverReturns
               WHERE WaterVehicleLoadingId = @WaterVehicleLoadingId AND FarmId = @FarmId
                 ...
                 AND (@ReplaceReturnId IS NULL OR WaterDriverReturnId <> @ReplaceReturnId))
    BEGIN
