-- =============================================================================
-- Migration 146: raw-material STOCK moves in PRODUCTION units, not purchase units
-- =============================================================================
-- Critical fix (James): recording a purchase increased CurrentQuantity by the
-- PURCHASE quantity, but production/usage DECREMENTS it by the PRODUCTION-level
-- quantity. With any unit conversion (e.g. 1 Roll = 100 m) the stock was wrong:
-- +1 on purchase, -100 on use. Stock must be tracked in PRODUCTION units so the
-- whole raw-material design is consistent.
--
-- The production-level quantity = purchase Quantity x ProductionUnitsPerPurchaseUnit
-- (a NULL/0 conversion means "same unit" -> multiplier 1, so purchases with no
-- conversion are unaffected). This migration redefines the Insert / Update /
-- Delete SPs for BOTH Water and Poultry raw materials to move stock by that
-- production-level quantity. Every SP is copied verbatim from its latest
-- definition (116 / 130 / 144) with ONLY the CurrentQuantity math changed.
--
-- NOTE: this corrects behaviour going forward. Existing CurrentQuantity values
-- recorded before this migration (for items whose purchases used a conversion
-- other than 1) may need a one-time recompute; that is handled separately after
-- review so no balances are silently rewritten here.
-- =============================================================================
...
BEGIN
    ...
    SET    CurrentQuantity = CurrentQuantity + @ProdQty, UpdatedAt = SYSUTCDATETIME()   -- 146: production-level qty
    WHERE  WaterRawMaterialItemId = @WaterRawMaterialItemId AND FarmId = @FarmId;
...
BEGIN
    ...
    DECLARE @OldQty DECIMAL(14,3), @ItemId INT, @LinkedExpenseId INT, @OldMult DECIMAL(14,4);
    ...
    -- 146: stock delta measured in PRODUCTION units (new prod qty - old prod qty).
    DECLARE @NewMult DECIMAL(14,4) = ISNULL(NULLIF(@ProductionUnitsPerPurchaseUnit, 0), 1);
    ...
    IF (@Delta <> 0)
        UPDATE dbo.WaterRawMaterialItems
        SET    CurrentQuantity = CurrentQuantity + @Delta, UpdatedAt = SYSUTCDATETIME()   -- 146: production-level delta
        WHERE  WaterRawMaterialItemId = @ItemId AND FarmId = @FarmId;
...
BEGIN
    ...
    DECLARE @Qty DECIMAL(14,3), @ItemId INT, @LinkedExpenseId INT, @Mult DECIMAL(14,4);
    ...
    DECLARE @ProdQty DECIMAL(18,4) = @Qty * @Mult;   -- 146
    DECLARE @CurrentStock DECIMAL(18,4) = (
        SELECT CurrentQuantity FROM dbo.WaterRawMaterialItems
        WHERE  WaterRawMaterialItemId = @ItemId AND FarmId = @FarmId);
    ...
    UPDATE dbo.WaterRawMaterialItems
    SET    CurrentQuantity = CurrentQuantity - @ProdQty,   -- 146: production-level qty
           UpdatedAt       = SYSUTCDATETIME()
    WHERE  WaterRawMaterialItemId = @ItemId AND FarmId = @FarmId;
...
BEGIN
    ...
    DECLARE @OldQty DECIMAL(14,3), @ItemId INT, @OldMult DECIMAL(14,4);
    ...
    -- 146: stock delta measured in PRODUCTION units.
    DECLARE @NewMult DECIMAL(14,4) = ISNULL(NULLIF(@ProductionUnitsPerPurchaseUnit, 0), 1);
    DECLARE @Delta DECIMAL(18,4) = (@Quantity * @NewMult) - (@OldQty * @OldMult);
    IF (@Delta < 0)
    BEGIN
    ...
    IF (@Delta <> 0)
        UPDATE dbo.PoultryRawMaterialItems SET CurrentQuantity = CurrentQuantity + @Delta, UpdatedAt = SYSUTCDATETIME()   -- 146: production-level delta
...
BEGIN
    ...
    DECLARE @Qty DECIMAL(14,3), @ItemId INT, @Mult DECIMAL(14,4);
    ...
    UPDATE dbo.PoultryRawMaterialItems
    SET    CurrentQuantity = CASE WHEN CurrentQuantity - @ProdQty < 0 THEN 0 ELSE CurrentQuantity - @ProdQty END, UpdatedAt = SYSUTCDATETIME()   -- 146
    WHERE  PoultryRawMaterialItemId = @ItemId AND FarmId = @FarmId;
    COMMIT TRANSACTION;
END
GO
...
PRINT '146_RawMaterialStockUsesProductionQuantity.sql complete.';
GO
