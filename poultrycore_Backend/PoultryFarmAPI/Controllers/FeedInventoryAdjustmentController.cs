...
namespace PoultryFarmAPIWeb.Controllers
{
    ...
    public class FeedInventoryAdjustmentController : ControllerBase
    {
        private readonly IFeedInventoryAdjustmentService _service;

        public FeedInventoryAdjustmentController(IFeedInventoryAdjustmentService service)
        {
            _service = service;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<List<FeedInventoryAdjustmentModel>>> GetAll([FromQuery] string farmId)
        {
        ...
        [HttpGet("{id:int}")]
        public async Task<ActionResult<FeedInventoryAdjustmentModel>> GetById(int id, [FromQuery] string farmId)
        {
            if (string.IsNullOrWhiteSpace(farmId))
                return BadRequest("FarmId is required.");
        ...
        [HttpPost]
        public async Task<ActionResult<FeedInventoryAdjustmentModel>> Create([FromBody] FeedInventoryAdjustmentModel model)
        {
            ...
            if (model.FeedDeltaKg == 0)
                return BadRequest("FeedDeltaKg cannot be zero.");
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] FeedInventoryAdjustmentModel model)
        {
            if (string.IsNullOrWhiteSpace(model.FarmId))
                return BadRequest("FarmId is required.");
            if (!ValidTypes.Contains(model.AdjustmentType, StringComparer.OrdinalIgnoreCase))
