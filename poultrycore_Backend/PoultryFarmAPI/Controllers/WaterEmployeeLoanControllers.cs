// Employee Loans & Advances, water side (migrations 313/314) + structured payroll
// deductions. Same flat per-resource [Route] with farmId on the query string
// that the rest of the water finance controllers use.
//
...
{
    [ApiController]
    [Route("api/Water/employee-loans")]
    public class WaterEmployeeLoanController : ControllerBase
    {
        private readonly IWaterEmployeeLoanService _svc;
        public WaterEmployeeLoanController(IWaterEmployeeLoanService svc) => _svc = svc;
        ...
        /// </summary>
        [HttpGet]
        public async Task<ActionResult<WaterEmployeeLoanPage>> GetAll(
        ...
        [HttpGet("summary")]
        public async Task<ActionResult<WaterEmployeeLoanSummary>> Summary(
        ...
        /// </summary>
        [HttpGet("eligible")]
        public async Task<ActionResult<IEnumerable<WaterEmployeeLoanEligible>>> Eligible(
        ...
        [HttpGet("{id:int}")]
        public async Task<ActionResult<WaterEmployeeLoanModel>> GetById(
        ...
        public async Task<ActionResult<IEnumerable<WaterEmployeeLoanRepaymentModel>>> Repayments(
            int id, [FromQuery] string farmId)
            => string.IsNullOrWhiteSpace(farmId)
                ? BadRequest("Company ID is required.")
        ...
        /// </summary>
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] WaterEmployeeLoanUpdateRequest r)
        {
            if (string.IsNullOrWhiteSpace(r.FarmId)) return BadRequest("Company ID is required.");
        ...
        public async Task<IActionResult> Disburse(
            int id, [FromBody] WaterEmployeeLoanDisburseRequest r)
        {
            if (string.IsNullOrWhiteSpace(r.FarmId)) return BadRequest("Company ID is required.");
            if (r.WaterCashAccountId <= 0)
                return BadRequest("Choose the cash account the money is coming out of.");
        ...
        public async Task<IActionResult> Cancel(int id, [FromBody] WaterEmployeeLoanReasonRequest r)
        {
            if (string.IsNullOrWhiteSpace(r.FarmId)) return BadRequest("Company ID is required.");
        ...
        public async Task<IActionResult> Reverse(int id, [FromBody] WaterEmployeeLoanReasonRequest r)
        {
            if (string.IsNullOrWhiteSpace(r.FarmId)) return BadRequest("Company ID is required.");
        ...
        public async Task<ActionResult<int>> RecordRepayment(
            [FromBody] WaterEmployeeLoanRepaymentRequest r)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
            if (string.IsNullOrWhiteSpace(r.FarmId)) return BadRequest("Company ID is required.");
            try
            {
                ...
                return Ok(new { WaterEmployeeLoanRepaymentId = id });
            }
        ...
        public async Task<IActionResult> ReverseRepayment(
            int repaymentId, [FromBody] WaterEmployeeLoanReasonRequest r)
        {
            if (string.IsNullOrWhiteSpace(r.FarmId)) return BadRequest("Company ID is required.");
    ...
    /// <summary>
    /// The detail behind a payroll line's Deductions figure (314).
    ///
    ...
    /// </summary>
    [ApiController]
    [Route("api/Water/payroll-deductions")]
    public class WaterPayrollDeductionController : ControllerBase
    {
        private readonly IWaterEmployeeLoanService _svc;
        public WaterPayrollDeductionController(IWaterEmployeeLoanService svc) => _svc = svc;
        ...
        /// </summary>
        [HttpGet]
        public async Task<ActionResult<IEnumerable<WaterPayrollDeductionModel>>> GetForItem(
        ...
        public async Task<ActionResult<IEnumerable<WaterPayrollDeductionRunRow>>> GetForRun(
        ...
        /// </summary>
        [HttpPost]
        public async Task<ActionResult<int>> Save([FromBody] WaterPayrollDeductionSaveRequest r)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
            if (string.IsNullOrWhiteSpace(r.FarmId)) return BadRequest("Company ID is required.");
            var id = await _svc.SaveDeductionAsync(r);
            return Ok(new { WaterPayrollItemDeductionId = id });
        }
