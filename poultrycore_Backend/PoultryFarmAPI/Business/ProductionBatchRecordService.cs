    ...
    public class ProductionBatchRecordService : IProductionBatchRecordService
    {
        ...
        private static string BuildAllocationsJson(IEnumerable<ProductionBatchAllocationModel> allocations)
            => JsonSerializer.Serialize(allocations.Select(a => new
            {
                ...
                fifthPickEggs = a.FifthPickEggs,
                sixthPickEggs = a.SixthPickEggs,
        ...
        private static void AddHeaderParams(NpgsqlCommand cmd, ProductionBatchRecordModel m)
        {
            ...
            cmd.Parameters.AddWithValue("@FifthPickCrates", (object?)m.FifthPickCrates ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@FifthPickLooseEggs", (object?)m.FifthPickLooseEggs ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@FifthPickTotal", m.FifthPickTotal);
            cmd.Parameters.AddWithValue("@SixthPickCrates", (object?)m.SixthPickCrates ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@SixthPickLooseEggs", (object?)m.SixthPickLooseEggs ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@SixthPickTotal", m.SixthPickTotal);
        ...
        private static ProductionBatchRecordModel MapHeader(NpgsqlDataReader reader, bool includeAllocations)
        {
            var m = new ProductionBatchRecordModel
            {
                ...
                FifthPickCrates = GetInt(reader, "FifthPickCrates"),
                FifthPickLooseEggs = GetInt(reader, "FifthPickLooseEggs"),
                FifthPickTotal = GetInt(reader, "FifthPickTotal") ?? 0,
                SixthPickCrates = GetInt(reader, "SixthPickCrates"),
                SixthPickLooseEggs = GetInt(reader, "SixthPickLooseEggs"),
                SixthPickTotal = GetInt(reader, "SixthPickTotal") ?? 0,
