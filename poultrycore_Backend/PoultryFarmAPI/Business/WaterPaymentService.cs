...
namespace PoultryFarmAPIWeb.Business
{
    public class WaterPaymentService : IWaterPaymentService
    {
        ...
        public async Task<int> Record(WaterPaymentModel m)
        {
            ...
            using var cmd = new NpgsqlCommand("SELECT * FROM spwaterpayment_record(p_farmid => @FarmId::text, p_watersaleid => @WaterSaleId::int, p_amount => @Amount::numeric, p_paymentmethod => @PaymentMethod::text, p_paymentdate => @PaymentDate::timestamp, p_reference => @Reference::text, p_note => @Note::text, p_createdby => @CreatedBy::text)", conn);
        ...
        public async Task<List<WaterPaymentModel>> GetBySale(int saleId, string farmId)
        {
            ...
            using var conn = new NpgsqlConnection(_connectionString);
            using var cmd = new NpgsqlCommand("SELECT * FROM spwaterpayment_getbysale(p_watersaleid => @WaterSaleId::int, p_farmid => @FarmId::text)", conn);
        ...
        public async Task<List<WaterPaymentModel>> GetAll(string farmId)
        {
            var list = new List<WaterPaymentModel>();
            using var conn = new NpgsqlConnection(_connectionString);
            using var cmd = new NpgsqlCommand("SELECT * FROM spwaterpayment_getall(p_farmid => @FarmId::text)", conn);
            cmd.Parameters.AddWithValue("@FarmId", farmId);
        ...
        private static WaterPaymentModel Read(NpgsqlDataReader r, bool includeCustomerName) => new()
        {
