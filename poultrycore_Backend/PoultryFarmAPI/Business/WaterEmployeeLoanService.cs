...
// and maps rows, exactly like WaterLoanService does for borrowing.
//
...
// create a repayment with no deduction behind it, which 317 would then have no
// way to reverse when the payroll was reopened.
//
// Identical in shape to PoultryEmployeeLoanService: the same fifteen methods
// over the same eleven functions. Two modules rather than one generic service,
// because water and poultry keep separate tables for everything -- a member of
// staff belongs to one company type, and an advance belongs to one member of
// staff.
...
{
    public interface IWaterEmployeeLoanService
    {
        Task<WaterEmployeeLoanPage> GetAllAsync(
        ...
        Task<WaterEmployeeLoanModel?> GetByIdAsync(int id, string farmId);
        Task<WaterEmployeeLoanSummary> GetSummaryAsync(string farmId, DateTime? from, DateTime? to);
        Task<List<WaterEmployeeLoanEligible>> GetEligibleAsync(string farmId, int staffId);

        Task<int> CreateAsync(WaterEmployeeLoanCreateRequest r);
        Task UpdateAsync(int id, WaterEmployeeLoanUpdateRequest r);
        Task DisburseAsync(int id, WaterEmployeeLoanDisburseRequest r);
        Task CancelAsync(int id, WaterEmployeeLoanReasonRequest r);
        Task ReverseAsync(int id, WaterEmployeeLoanReasonRequest r);

        Task<List<WaterEmployeeLoanRepaymentModel>> GetRepaymentsAsync(string farmId, int loanId);
        Task<int> RecordRepaymentAsync(WaterEmployeeLoanRepaymentRequest r);
        Task ReverseRepaymentAsync(int repaymentId, WaterEmployeeLoanReasonRequest r);

        // ---- structured payroll deductions (314) ----
        Task<List<WaterPayrollDeductionModel>> GetDeductionsAsync(string farmId, int payrollItemId);
        Task<List<WaterPayrollDeductionRunRow>> GetRunDeductionsAsync(string farmId, int runId);
        Task<int> SaveDeductionAsync(WaterPayrollDeductionSaveRequest r);
        Task DeleteDeductionAsync(int deductionId, string farmId, string? deletedBy);
    }

    public class WaterEmployeeLoanService : IWaterEmployeeLoanService
    {
        private readonly string _cs;
        public WaterEmployeeLoanService(string cs) => _cs = cs;
        ...
        private static WaterEmployeeLoanModel ReadLoan(NpgsqlDataReader r) => new()
        {
            WaterEmployeeLoanId = r.GetInt32(r.GetOrdinal("WaterEmployeeLoanId")),
        ...
        private static WaterEmployeeLoanRepaymentModel ReadRepayment(NpgsqlDataReader r) => new()
        {
            WaterEmployeeLoanRepaymentId = r.GetInt32(r.GetOrdinal("WaterEmployeeLoanRepaymentId")),
            WaterEmployeeLoanId = r.GetInt32(r.GetOrdinal("WaterEmployeeLoanId")),
            WaterStaffId = r.GetInt32(r.GetOrdinal("WaterStaffId")),
            StaffName = Str(r, "StaffName"),
            ...
            WaterPayrollRunId = IntN(r, "WaterPayrollRunId"),
            PayrollPeriodStart = DateN(r, "PayrollPeriodStart"),
            PayrollPeriodEnd = DateN(r, "PayrollPeriodEnd"),
            WaterPayrollItemId = IntN(r, "WaterPayrollItemId"),
            WaterPayrollDeductionId = IntN(r, "WaterPayrollDeductionId"),
            WaterCashAccountId = IntN(r, "WaterCashAccountId"),
            CashAccountName = Str(r, "CashAccountName"),
            PaymentMethod = Str(r, "PaymentMethod"),
            ReferenceNumber = Str(r, "ReferenceNumber"),
        ...
        // Reads
        // ==================================================================
        public async Task<WaterEmployeeLoanPage> GetAllAsync(
            string farmId, int? staffId, string? loanType, string? status,
            string? repaymentMethod, DateTime? from, DateTime? to, string? search,
            int limit, int offset)
        {
            var page = new WaterEmployeeLoanPage();
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT * FROM spwateremployeeloan_getall(" +
                "p_farmid => @FarmId::text, p_waterstaffid => @StaffId::int, " +
        ...
        public async Task<WaterEmployeeLoanModel?> GetByIdAsync(int id, string farmId)
        {
            ...
            return page.Items.FirstOrDefault(l => l.WaterEmployeeLoanId == id);
        }

        public async Task<WaterEmployeeLoanSummary> GetSummaryAsync(
            string farmId, DateTime? from, DateTime? to)
        {
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT * FROM spwateremployeeloan_summary(" +
            ...
            await c.OpenAsync();
            using var r = await cmd.ExecuteReaderAsync();
            if (!await r.ReadAsync()) return new WaterEmployeeLoanSummary();
            return new WaterEmployeeLoanSummary
            {
        ...
        public async Task<List<WaterEmployeeLoanEligible>> GetEligibleAsync(string farmId, int staffId)
        {
            var list = new List<WaterEmployeeLoanEligible>();
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT * FROM spwateremployeeloan_eligible(" +
                "p_farmid => @FarmId::text, p_waterstaffid => @StaffId::int)", c);
            cmd.Parameters.AddWithValue("@FarmId", farmId);
            cmd.Parameters.AddWithValue("@StaffId", staffId);
            await c.OpenAsync();
            using var r = await cmd.ExecuteReaderAsync();
            while (await r.ReadAsync())
                list.Add(new WaterEmployeeLoanEligible
                {
                    WaterEmployeeLoanId = r.GetInt32(r.GetOrdinal("WaterEmployeeLoanId")),
                    LoanNumber = Str(r, "LoanNumber"),
                    LoanType = Str(r, "LoanType") ?? "EmployeeLoan",
        ...
        public async Task<List<WaterEmployeeLoanRepaymentModel>> GetRepaymentsAsync(
            string farmId, int loanId)
        {
            var list = new List<WaterEmployeeLoanRepaymentModel>();
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT * FROM spwateremployeeloanrepayment_getall(" +
                "p_farmid => @FarmId::text, p_wateremployeeloanid => @LoanId::int)", c);
            cmd.Parameters.AddWithValue("@FarmId", farmId);
        ...
        // Writes
        // ==================================================================
        public async Task<int> CreateAsync(WaterEmployeeLoanCreateRequest q)
        {
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT * FROM spwateremployeeloan_create(" +
                "p_farmid => @FarmId::text, p_waterstaffid => @StaffId::int, " +
                ...
                "p_watercashaccountid => @CashAccountId::int, p_paymentmethod => @PayMethod::text, " +
                "p_referencenumber => @Reference::text, p_createdby => @CreatedBy::text)", c);
            cmd.Parameters.AddWithValue("@FarmId", q.FarmId);
            cmd.Parameters.AddWithValue("@StaffId", q.WaterStaffId);
            ...
            cmd.Parameters.AddWithValue("@CashAccountId", Db(q.WaterCashAccountId));
        ...
        public async Task UpdateAsync(int id, WaterEmployeeLoanUpdateRequest q)
        {
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT spwateremployeeloan_update(" +
                "p_farmid => @FarmId::text, p_wateremployeeloanid => @Id::int, " +
                "p_principalamount => @Principal::numeric, p_disbursementdate => @DisbDate::date, " +
                "p_loantype => @LoanType::text, p_interestenabled => @IntOn::boolean, " +
                "p_interestamount => @IntAmt::numeric, p_interestrate => @IntRate::numeric, " +
                "p_interesttype => @IntType::text, p_repaymentmethod => @RepayMethod::text, " +
        ...
        public async Task DisburseAsync(int id, WaterEmployeeLoanDisburseRequest q)
        {
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT spwateremployeeloan_disburse(" +
                "p_farmid => @FarmId::text, p_wateremployeeloanid => @Id::int, " +
                "p_watercashaccountid => @CashAccountId::int, p_paymentmethod => @PayMethod::text, " +
                "p_referencenumber => @Reference::text, p_disbursedby => @By::text)", c);
            cmd.Parameters.AddWithValue("@FarmId", q.FarmId);
            cmd.Parameters.AddWithValue("@Id", id);
            cmd.Parameters.AddWithValue("@CashAccountId", q.WaterCashAccountId);
            cmd.Parameters.AddWithValue("@PayMethod", Db(q.PaymentMethod));
            cmd.Parameters.AddWithValue("@Reference", Db(q.ReferenceNumber));
        ...
        public async Task CancelAsync(int id, WaterEmployeeLoanReasonRequest q)
        {
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT spwateremployeeloan_cancel(" +
                "p_farmid => @FarmId::text, p_wateremployeeloanid => @Id::int, " +
        ...
        public async Task ReverseAsync(int id, WaterEmployeeLoanReasonRequest q)
        {
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT spwateremployeeloan_reverse(" +
                "p_farmid => @FarmId::text, p_wateremployeeloanid => @Id::int, " +
        ...
        public async Task<int> RecordRepaymentAsync(WaterEmployeeLoanRepaymentRequest q)
        {
            ...
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT * FROM spwateremployeeloanrepayment_record(" +
                "p_farmid => @FarmId::text, p_wateremployeeloanid => @LoanId::int, " +
                "p_amount => @Amount::numeric, p_sourcetype => @Source::text, " +
                "p_principalamount => @Principal::numeric, p_interestamount => @Interest::numeric, " +
                "p_repaymentdate => @Date::timestamp, p_watercashaccountid => @CashAccountId::int, " +
            ...
            cmd.Parameters.AddWithValue("@FarmId", q.FarmId);
            cmd.Parameters.AddWithValue("@LoanId", q.WaterEmployeeLoanId);
        ...
        public async Task ReverseRepaymentAsync(int repaymentId, WaterEmployeeLoanReasonRequest q)
        {
            using var c = new NpgsqlConnection(_cs);
            ...
            using var cmd = new NpgsqlCommand(
                "SELECT spwateremployeeloanrepayment_reverse(" +
                "p_farmid => @FarmId::text, " +
                "p_wateremployeeloanrepaymentid => @Id::int, " +
                "p_reversedby => @By::text, p_reason => @Reason::text)", c);
            cmd.Parameters.AddWithValue("@FarmId", q.FarmId);
        ...
        // Structured payroll deductions (314)
        // ==================================================================
        public async Task<List<WaterPayrollDeductionModel>> GetDeductionsAsync(
            string farmId, int payrollItemId)
        {
            var list = new List<WaterPayrollDeductionModel>();
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT * FROM spwaterpayrollitemdeduction_getall(" +
                "p_farmid => @FarmId::text, p_waterpayrollitemid => @ItemId::int)", c);
            cmd.Parameters.AddWithValue("@FarmId", farmId);
            cmd.Parameters.AddWithValue("@ItemId", payrollItemId);
            await c.OpenAsync();
            using var r = await cmd.ExecuteReaderAsync();
            while (await r.ReadAsync())
                list.Add(new WaterPayrollDeductionModel
                {
                    WaterPayrollItemDeductionId = IntN(r, "WaterPayrollItemDeductionId"),
                    WaterPayrollItemId = r.GetInt32(r.GetOrdinal("WaterPayrollItemId")),
                    WaterStaffId = r.GetInt32(r.GetOrdinal("WaterStaffId")),
                    DeductionType = Str(r, "DeductionType") ?? "OtherDeduction",
                    Amount = r.GetDecimal(r.GetOrdinal("Amount")),
                    WaterEmployeeLoanId = IntN(r, "WaterEmployeeLoanId"),
                    LoanNumber = Str(r, "LoanNumber"),
                    LoanType = Str(r, "LoanType"),
                    LoanOutstanding = DecN(r, "LoanOutstanding"),
                    WaterEmployeeLoanRepaymentId = IntN(r, "WaterEmployeeLoanRepaymentId"),
                    Description = Str(r, "Description"),
        ...
        public async Task<List<WaterPayrollDeductionRunRow>> GetRunDeductionsAsync(
            string farmId, int runId)
        {
            var list = new List<WaterPayrollDeductionRunRow>();
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT * FROM spwaterpayrollitemdeduction_getforrun(" +
                "p_farmid => @FarmId::text, p_waterpayrollrunid => @RunId::int)", c);
            cmd.Parameters.AddWithValue("@FarmId", farmId);
            cmd.Parameters.AddWithValue("@RunId", runId);
            await c.OpenAsync();
            using var r = await cmd.ExecuteReaderAsync();
            while (await r.ReadAsync())
                list.Add(new WaterPayrollDeductionRunRow
                {
                    WaterPayrollItemId = r.GetInt32(r.GetOrdinal("WaterPayrollItemId")),
                    WaterStaffId = r.GetInt32(r.GetOrdinal("WaterStaffId")),
                    StaffName = Str(r, "StaffName"),
        ...
        public async Task<int> SaveDeductionAsync(WaterPayrollDeductionSaveRequest q)
        {
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT * FROM spwaterpayrollitemdeduction_save(" +
                "p_farmid => @FarmId::text, p_waterpayrollitemid => @ItemId::int, " +
                "p_deductiontype => @Type::text, p_amount => @Amount::numeric, " +
                "p_wateremployeeloanid => @LoanId::int, p_description => @Description::text, " +
                "p_reference => @Reference::text, " +
                "p_waterpayrollitemdeductionid => @Id::int, p_savedby => @By::text)", c);
            cmd.Parameters.AddWithValue("@FarmId", q.FarmId);
            cmd.Parameters.AddWithValue("@ItemId", q.WaterPayrollItemId);
            cmd.Parameters.AddWithValue("@Type", q.DeductionType);
            cmd.Parameters.AddWithValue("@Amount", q.Amount);
            cmd.Parameters.AddWithValue("@LoanId", Db(q.WaterEmployeeLoanId));
            cmd.Parameters.AddWithValue("@Description", Db(q.Description));
            cmd.Parameters.AddWithValue("@Reference", Db(q.Reference));
            cmd.Parameters.AddWithValue("@Id", Db(q.WaterPayrollItemDeductionId));
        ...
        public async Task DeleteDeductionAsync(int deductionId, string farmId, string? deletedBy)
        {
            using var c = new NpgsqlConnection(_cs);
            using var cmd = new NpgsqlCommand(
                "SELECT spwaterpayrollitemdeduction_delete(" +
                "p_farmid => @FarmId::text, p_waterpayrollitemdeductionid => @Id::int, " +
