...
namespace PoultryFarmAPIWeb.Business
{
    ...
    public class RestaurantGiftCardService : IRestaurantGiftCardService
    {
        ...
        public async Task ReloadAsync(string cardNumber, string farmId, decimal amount, string? processedBy = null, string paymentMethod = "Cash", int? cashAccountId = null) { using var c=new NpgsqlConnection(_cs); using var cmd=new NpgsqlCommand("SELECT sprestaurant_giftcard_reload(p_cardnumber=>@N::text,p_farmid=>@F::text,p_amount=>@A::numeric,p_processedby=>@P::text,p_paymentmethod=>@pm::text,p_cashaccountid=>@ca::int)",c); cmd.Parameters.AddWithValue("@pm",string.IsNullOrWhiteSpace(paymentMethod)?"Cash":paymentMethod); cmd.Parameters.AddWithValue("@ca",(object?)cashAccountId??DBNull.Value); cmd.Parameters.AddWithValue("@N",cardNumber); cmd.Parameters.Add(TP("@F",farmId)); cmd.Parameters.AddWithValue("@A",amount); cmd.Parameters.AddWithValue("@P",(object?)processedBy??DBNull.Value); await c.OpenAsync(); await cmd.ExecuteNonQueryAsync(); }

        public async Task<GiftCardModel?> CheckBalanceAsync(string cardNumber, string farmId) { using var c=new NpgsqlConnection(_cs); using var cmd=new NpgsqlCommand("SELECT * FROM sprestaurant_giftcard_balance(p_cardnumber=>@N::text,p_farmid=>@F::text)",c); cmd.Parameters.AddWithValue("@N",cardNumber); cmd.Parameters.Add(TP("@F",farmId)); await c.OpenAsync(); using var r=await cmd.ExecuteReaderAsync(); if(!await r.ReadAsync()) return null; return new(){CardNumber=r.GetString(0),CurrentBalance=r.GetDecimal(1),Status=r.GetString(2),ExpiryDate=r.IsDBNull(3)?null:r.GetDateTime(3)}; }
    ...
    public class RestaurantExpenseService : IRestaurantExpenseService
    {
        ...
        // Through sprestaurant_expense_record (migration 323), which writes the
        // expense and its cash-ledger row in one call and refuses a closed day. The
        // old inline INSERT moved no money, so expenses never touched an account.
